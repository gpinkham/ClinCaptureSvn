...
public class OnChangeSheetValidator implements SpreadSheetValidator {
	...
	/*
	 * If onChangeTypeFirst of cell is true, validate cell's OnChangeValidationType first, then cell's
	...
	private void validateCellResValueAll(OnChangeSheetValidationCell onChangeSheetValidationCell) {
		...
		if (cell.getColValue() == null || cell.getColValue().isEmpty()) {
			StringBuffer message = sheetErrors.errorMessage(resPageMsg.getString(cell.getColTitle()),
					resPageMsg.getString(SheetValidationType.IS_REQUIRED.getDescription()),
			...
			if (!OnChangeSheetValidator.isValidInstantOnChangeSyntax(cell.getColValue())) {
				StringBuffer message = sheetErrors.errorMessage(resPageMsg.getString(cell.getColTitle()),
						resPageMsg.getString(OnChangeSheetValidationType.SHOULD_BE_FUNC_ONCHANGE.getDescription()),
						resPageMsg.getString(cell.getForWhich()));
				sheetErrors.addError(cell.getRowNum(), message);
				printHtmlErr = true;
			} else {
				...
				if (!OnChangeSheetValidator.isValidInstantOnChangeType(onChangeSheetValidationCell)) {
					StringBuffer message = sheetErrors.errorMessage(resPageMsg.getString(cell.getColTitle()),
							resPageMsg.getString(OnChangeSheetValidationType.SHOULD_BE_FUNC_ONCHANGE_TYPE
									.getDescription()), resPageMsg.getString(cell.getForWhich()));
					sheetErrors.addError(cell.getRowNum(), message);
					printHtmlErr = true;
				}
				// break;
		...
		// if(printHtmlErr)
		// errors.putHtmlError(cell.getSheetNum(), cell.getRowNum(), cell.getColNum(),
	...
	private boolean isValidInstantOnChangeItemPair(OnChangeSheetValidationCell onChangeSheetValidationCell) {
		...
		if (cell.getColValue() != null && cell.getColValue().length() > 0
				&& allItems.containsKey(onChangeSheetValidationCell.getOriName())) {
			if (sheetValidationContainer.inSameSection(cell.getRowName(), onChangeSheetValidationCell.getOriName())) {
				if (isValidInstantOnChangeGroupPair(onChangeSheetValidationCell.getOriName(), cell.getRowName())) {
					...
					s.append(" ");
					s.append(resPageMsg.getString(OnChangeSheetValidationType.SHOULD_IN_SAME_REPEATING_GROUP
							.getDescription()));
					StringBuffer message = sheetErrors.errorMessage(resPageMsg.getString(cell.getColTitle()),
							s.toString(), resPageMsg.getString(cell.getForWhich()));
					sheetErrors.addError(cell.getRowNum(), message);
					sheetErrors.putHtmlError(cell.getSheetNum(), cell.getRowNum(), 6,
							resPageMsg.getString("INVALID_FIELD"));
				}
			} else {
				...
				sheetErrors.addError(cell.getRowNum(), message);
				sheetErrors
						.putHtmlError(cell.getSheetNum(), cell.getRowNum(), 5, resPageMsg.getString("INVALID_FIELD"));
			}
		} else {
			StringBuffer s = new StringBuffer(onChangeSheetValidationCell.getOriName());
			s.append(" ");
			...
			StringBuffer message = sheetErrors.errorMessage(resPageMsg.getString(cell.getColTitle()), s.toString(),
					resPageMsg.getString(cell.getForWhich()));
			sheetErrors.addError(cell.getRowNum(), message);
			if (!sheetErrors.htmlErrors.containsKey(sheetErrors.htmlErrorKey(cell.getSheetNum(), cell.getRowNum(),
					cell.getColNum())))
				sheetErrors.putHtmlError(cell.getSheetNum(), cell.getRowNum(), cell.getColNum(),
						resPageMsg.getString("INVALID_FIELD"));
		}
	...
	public boolean isValidInstantOnChangeGroupPair(String oriName, String destName) {
		if (!sheetValidationContainer.inRepeatingGroup(oriName) && !sheetValidationContainer.inRepeatingGroup(destName)) {
			return true;
		} else {
