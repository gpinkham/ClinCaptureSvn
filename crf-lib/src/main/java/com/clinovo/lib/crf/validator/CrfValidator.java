 ...
 * CrfValidator.
 */
public final class CrfValidator {
		...
		if (crfBuilder.getCurrentItem().getResponseSet().getResponseType()
			...
			if (crfBuilder.getCurrentItem().getResponseSet().getOptionsValues().contains(":")) {
				...
				if (s.length > 0 && !FUNC.equalsIgnoreCase(s[0].trim())) {
					crfBuilder.getErrorMessageProducer().expressionStartsWithFunc();
				}
			}
			...
			if (!(exp.startsWith("getexternalvalue") || exp.startsWith("getExternalValue"))) {
				...
				for (String variable : variables) {
					crfBuilder.setCurrentVariable(variable);
	...
	private static void addSpreadSheetItem(BaseCrfBuilder crfBuilder) throws Exception {
		crfBuilder.getSpreadSheetItems().add(new SpreadSheetItemUtil());
		crfBuilder.getLastSpreadSheetItem().setItemName(crfBuilder.getCurrentItem().getName());
		crfBuilder.getLastSpreadSheetItem().setDescriptionLabel(crfBuilder.getCurrentItem().getDescription());
		crfBuilder.getLastSpreadSheetItem()
				.setLeftItemText(crfBuilder.getCurrentItem().getItemMeta().getLeftItemText());
		crfBuilder.getLastSpreadSheetItem().setSectionLabel(crfBuilder.getCurrentItem().getItemMeta().getSectionName());
		crfBuilder.getLastSpreadSheetItem().setGroupLabel(crfBuilder.getCurrentItem().getItemMeta().getGroupLabel());
		crfBuilder.getLastSpreadSheetItem().setParentItem(crfBuilder.getCurrentItem().getItemMeta().getParentLabel());
		crfBuilder.getLastSpreadSheetItem()
				.setResponseTypeId(crfBuilder.getCurrentItem().getResponseSet().getResponseType().getResponseTypeId());
		crfBuilder.getLastSpreadSheetItem().setResponseOptions(
				crfBuilder.split(crfBuilder.getCurrentItem().getResponseSet().getOptionsText(), ","));
		crfBuilder.getLastSpreadSheetItem()
				.setDataType(crfBuilder.getCurrentItem().getRealValue(RealValueKey.ITEM_DATA_TYPE));
		crfBuilder.getLastSpreadSheetItem()
				.setDefaultValue(crfBuilder.getCurrentItem().getItemMeta().getDefaultValue());
	}

	private static void validateItemDisplayStatus(BaseCrfBuilder crfBuilder) throws Exception {
		if (!StringUtil.isBlank(crfBuilder.getCurrentItem().getRealValue(RealValueKey.SCD_DATA))) {
			...
			if (crfBuilder.getCurrentItem().getSimpleConditionalDisplayBean() != null) {
				if (crfBuilder.getCurrentItem().getSimpleConditionalDisplayBean().getControlItemName().length() > 0
					...
					if (SpreadSheetItemUtil.isItemWithSameParameterExistsIncludingMyself(
							crfBuilder.getCurrentItem().getSimpleConditionalDisplayBean().getControlItemName(),
							crfBuilder.getSpreadSheetItems())) {
	...
	private static void instantValidation(BaseCrfBuilder crfBuilder) {
		SpreadSheetItemUtil.verifySectionGroupPlacementForItems(crfBuilder.getSpreadSheetItems(),
				crfBuilder.getErrorsList(), crfBuilder.getErrorsMap(), crfBuilder.getCurrentItem().getSheetNumber(),
				crfBuilder.getPageMessagesResourceBundle(), crfBuilder.getItemGroupLabelMap());

		SpreadSheetItemUtil.verifyUniqueItemPlacementInGroups(crfBuilder.getSpreadSheetItems(),
				crfBuilder.getErrorsList(), crfBuilder.getErrorsMap(), crfBuilder.getCurrentItem().getSheetNumber(),
				crfBuilder.getPageMessagesResourceBundle(), crfBuilder.getCrfBean().getName(),
				crfBuilder.getDataSource());
	...
	public static void validate(BaseCrfBuilder crfBuilder) throws Exception {
		...
		for (ItemBean itemBean : crfBuilder.getItems()) {
			...
			addSpreadSheetItem(crfBuilder);
			validateItemDisplayStatus(crfBuilder);
		}
