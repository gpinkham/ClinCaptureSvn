...
public class StudyEventDefinitionEndpoint {
	...
	public Source getStudyList(@XPathParam("//sed:studyEventDefinitionListAll") NodeList studyNodeList)
			throws Exception {
		...
		StudyEventDefinitionRequestValidator studyEventDefinitionRequestValidator = new StudyEventDefinitionRequestValidator(
				dataSource);
		...
		if (!errors.hasErrors()) {
			...
			return new DOMSource(mapFailConfirmation(
					messages.getMessage("studyEventDefinitionEndpoint.fail", null, "Fail", locale), errors));
		}
	}
	...
	private BaseStudyDefinitionBean unMarshallRequest(Element studyEventDefinitionListAll) {
		...
		Element siteIdentifierElement = siteRef == null ? null : DomUtils.getChildElementByTagName(siteRef,
				"identifier");
	...
	private Element mapConfirmation(StudyBean study, String confirmation) throws Exception {
		...
		for (int index = 0; index < eventList.size(); index++) {
			...
			List<EventDefinitionCRFBean> eventCrfs = (List<EventDefinitionCRFBean>) getEventDefinitionCRFDao()
					.findAllByDefinition(study, event.getId());
			...
			for (int i = 0; i < eventCrfs.size(); i++) {
				...
				CRFVersionBean crfVersionBean = (CRFVersionBean) getCrfVersionDao().findByPK(
						eventCrf.getDefaultVersionId());
				element = document.createElementNS(NAMESPACE_URI_V1, "oid");
	...
	public EventDefinitionCRFDAO getEventDefinitionCRFDao() {
		eventDefinitionCRFDao = eventDefinitionCRFDao != null ? eventDefinitionCRFDao : new EventDefinitionCRFDAO(
				dataSource);
