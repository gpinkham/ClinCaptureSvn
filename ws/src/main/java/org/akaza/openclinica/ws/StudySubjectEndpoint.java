...
public class StudySubjectEndpoint {
	...
	public Source createStudySubject(@XPathParam("//studySubject:studySubject") NodeList subject) throws Exception {
		...
		try {
			...
			npe.printStackTrace();
			return new DOMSource(mapConfirmation(
					messages.getMessage("studySubjectEndpoint.fail", null, "Null Pointer Exception", locale), null,
					errors));
		} catch (Exception e) {
			List<String> error_messages = new ArrayList<String>();
			error_messages.add(e.getMessage());
			return new DOMSource(mapConfirmation(
					messages.getMessage("studySubjectEndpoint.fail", null, "Fail", locale), null, errors, "label",
					error_messages));
		}
	}
	...
	public ListAllByStudyResponse listStudySubjectsInStudy(JAXBElement<ListStudySubjectsInStudyType> requestElement)
			throws Exception {
		try {
			...
			return mapListStudySubjectsInStudyResponse(study,
					messages.getMessage("studySubjectEndpoint.success", null, "Success", locale),
	...
	public Source isStudySubject(@XPathParam("//studySubject:studySubject") NodeList subject) throws Exception {
		...
		if (!errors.hasErrors()) {
			...
			return new DOMSource(mapConfirmation(
					messages.getMessage("studySubjectEndpoint.success", null, "Success", locale),
					subjectStudyBean.getSubjectOIDId(), errors, "subjectOID"));
	 ...
	 * @throws Exception
	 */
	private ListAllByStudyResponse mapListStudySubjectsInStudyResponse(StudyBean study, String message,
			StudyRefType studyRef) throws Exception {
		ListAllByStudyResponse response = new ListAllByStudyResponse();
	...
	private StudyBean validateRequestAndReturnStudy(StudyRefType studyRef) {
		...
		if (studyIdentifier == null && siteIdentifier == null) {
			throw new OpenClinicaSystemException("studySubjectEndpoint.provide_valid_study_site",
					"Provide a valid study/site.");
		}
		if (studyIdentifier != null && siteIdentifier == null) {
			StudyBean study = getStudyDao().findByUniqueIdentifier(studyIdentifier);
			if (study == null) {
				throw new OpenClinicaSystemException("studySubjectEndpoint.invalid_study_identifier",
						"The study identifier you provided is not valid.");
			}
			StudyUserRoleBean studySur = getUserAccountDao().findRoleByUserNameAndStudyId(getUserAccount().getName(),
					study.getId());
		...
		if (studyIdentifier != null && siteIdentifier != null) {
			...
			StudyUserRoleBean siteSur = getUserAccountDao().findRoleByUserNameAndStudyId(getUserAccount().getName(),
					site.getId());
	...
	private SubjectTransferBean unMarshallToSubjectTransfer(Element subjectElement) throws ParseException, Exception {
		...
		subjectTransferBean.setDateOfBirth((dateOfBirthValue == null || dateOfBirthValue.length() == 0) ? null
				: getDate(dateOfBirthValue));
	...
	private SubjectStudyDefinitionBean unMarshallToSubjectStudy(Element subjectStudyElement) throws ParseException {
		...
		SubjectStudyDefinitionBean subjectTransferBean = new SubjectStudyDefinitionBean(studyIdentifier,
				siteIdentifier, getUserAccount(), studySubjectIdValue);
	...
	private String createSubject(SubjectTransferBean subjectTransfer) {
		...
		return this.subjectService.createSubject(subject, subjectTransfer.getStudy(),
				subjectTransfer.getEnrollmentDate(), subjectTransfer.getSecondaryId());
	}
	...
	private Element mapConfirmation(String confirmation, String studySubjectLabel, Errors errors, String label)
			throws Exception {
		return mapConfirmation(confirmation, studySubjectLabel, errors, label, null);
	}

	private Element mapConfirmation(String confirmation, String studySubjectLabel, Errors errors, String label,
			List<String> error_messages) throws Exception {
	...
	/**
	 * Helper method that translates a date object to an XMLGregorianCalendar which is the data type used in jaxb
	 * generated classes.
	 * 
	 * @param date
	...
	private XMLGregorianCalendar getXMLGregorianCalendarDate(Date date) throws Exception {
		...
		XMLGregorianCalendar gcDate = df.newXMLGregorianCalendarDate(gc.get(Calendar.YEAR), gc.get(Calendar.MONTH) + 1,
				gc.get(Calendar.DAY_OF_MONTH), DatatypeConstants.FIELD_UNDEFINED);
		return gcDate;
	}

	/**
	 * Helper method that translates a date object (specifically time) to an XMLGregorianCalendar which is the data type
	 * used in jaxb generated classes.
	 * 
	 * @param date
	 * @return XMLGregorianCalendar
	 * @throws Exception
	...
	private XMLGregorianCalendar getXMLGregorianCalendarTime(Date date) throws Exception {
		...
		gc.setTime(date);
		DatatypeFactory df = DatatypeFactory.newInstance();
		XMLGregorianCalendar gcTime = df.newXMLGregorianCalendarTime(gc.get(Calendar.HOUR_OF_DAY),
				gc.get(Calendar.MINUTE), gc.get(Calendar.SECOND), null, DatatypeConstants.FIELD_UNDEFINED);
		return gcTime;
	}
