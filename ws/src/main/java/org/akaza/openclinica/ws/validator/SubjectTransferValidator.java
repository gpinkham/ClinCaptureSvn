...
public class SubjectTransferValidator implements Validator {
	...
	public void validateIsSubjectExists(Object obj, Errors e) {
		...
		if (subjectStudyBean.getSiteUniqueId() != null) {
			site = helper.verifySite(getStudyDAO(), subjectStudyBean.getStudyUniqueId(),
					subjectStudyBean.getSiteUniqueId(), included_status, e);
			if (site == null) {
				return;
			}
			site_id = site.getId();
		...
		boolean isRoleVerified = helper.verifyRole(subjectStudyBean.getUser(), study.getId(), site_id, e);// .verifyUser(subjectStudyBean.getUser(),
																											// getUserAccountDAO(),
																											// study.getId(),
																											// site_id,
																											// e) ;
		...
		// if (study == null) {
		// e.reject("subjectTransferValidator.study_does_not_exist", new Object[] { subjectStudyBean.getStudyUniqueId()
		// }, "Study identifier you specified "
		...
		// StudyBean site = null;
		// if (subjectStudyBean.getSiteUniqueId() != null) {
		// site = getStudyDAO().findSiteByUniqueIdentifier(subjectStudyBean.getStudyUniqueId(),
		// subjectStudyBean.getSiteUniqueId());
		// if (site == null) {
		...
		StudySubjectBean studySubject = getStudySubjectDAO().findByLabelAndStudy(subjectStudyBean.getSubjectLabel(),
				subjectStudyBean.getStudy());
		...
		if (studySubject == null || studySubject.getOid() == null) {
			e.reject("studyEventTransferValidator.study_subject_does_not_exist",
					new Object[] { subjectStudyBean.getSubjectLabel(), subjectStudyBean.getStudy().getName() },
					"StudySubject label you specified " + subjectStudyBean.getSubjectLabel()
							+ " does not correspond to a study " + subjectStudyBean.getStudy().getName());
	...
	public void validate(Object obj, Errors e) {
		...
		// if (study == null) {
		// e.reject("subjectTransferValidator.study_does_not_exist", new Object[] { subjectTransferBean.getStudyOid() },
		// "Study identifier you specified "
		...
		// e.reject("subjectTransferValidator.study_status_wrong", new Object[] { subjectTransferBean.getStudyOid() },
		// "Study "
		// + subjectTransferBean.getStudyOid()
		// +" has wrong status. Subject can be added to an 'AVAILABLE' study only.");
		// return;
		// }
		// subjectTransferBean.setStudy(study);
		// }
		...
		// StudyBean site = null;
		// if (subjectTransferBean.getSiteIdentifier() != null) {
		// site = getStudyDAO().findSiteByUniqueIdentifier(subjectTransferBean.getStudyOid(),
		...
		//
		// e.reject("subjectTransferValidator.site_status_wrong", new Object[] { subjectTransferBean.getSiteIdentifier()
		// }, "Site "
		...
		if (subjectTransferBean.getPersonId() != null && subjectTransferBean.getPersonId().length() > 0) {
			SubjectBean subjectWithSamePersonId = getSubjectDao().findByUniqueIdentifier(
					subjectTransferBean.getPersonId());
		...
		if (studySubjectId == null || studySubjectId.length() < 1) {
			...
			e.reject("subjectTransferValidator.studySubjectId_invalid_length", new Object[] { studySubjectId },
					"studySubjectId: " + studySubjectId + " cannot be longer than 30 characters.");
			return;
		} else { // checks whether there is a subject with same id inside current study
			...
			if (subjectWithSame.getLabel().equals(studySubjectId)) {
				e.reject("subjectTransferValidator.subject_duplicated_label",
						new Object[] { studySubjectId, study.getIdentifier() }, "studySubjectId: " + studySubjectId
