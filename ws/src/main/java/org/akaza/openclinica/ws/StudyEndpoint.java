...
public class StudyEndpoint {
	...
	public StudyEndpoint(DataSource dataSource, MessageSource messages, CoreResources coreResources,
	...
	public Source getStudyMetadata(@XPathParam("//study:studyMetadata") NodeList studyNodeList) throws Exception {
		...
		if (!errors.hasErrors()) {
			...
			return new DOMSource(mapConfirmation(messages.getMessage("studyEndpoint.fail", null, "Fail", locale),
					errors));
		}
	}
	...
	private HashMap<Integer, ArrayList<StudyBean>> getStudies() {
		...
		ArrayList<StudyUserRoleBean> studyUserRoleBeans = getUserAccountDao().findStudyByUser(
				getUserAccount().getName(), (ArrayList) getStudyDao().findAll());
