			...
			while (displayIt.hasNext()) {
				...
				if (count == displayCRFs.size() - 1 && dseBean.getUncompletedCRFs().size() == 0) {
					...
					if (ecb != null && ecb.getId() == dec.getEventCRF().getId()) {// was
						...
						displayData.add(new StudyInfoPanelLine("" + getStageImageText(dec.getStage()), "<a href='"
								+ getTOCLink(dec) + "'>" + dec.getEventCRF().getCrf().getName() + " "
					...
					if (ecb != null && ecb.getId() == dedc.getEventCRF().getId()
						...
						displayData.add(new StudyInfoPanelLine(
								...
								"<a href='InitialDataEntry?eventDefinitionCRFId=" + dedc.getEdc().getId()
										+ "&studyEventId=" + seBean.getId() + "&subjectId="
										+ studySubject.getSubjectId() + "&eventCRFId=" + dedc.getEventCRF().getId()
										+ "&crfVersionId=" + dedc.getEdc().getDefaultVersionId() + "'>"
