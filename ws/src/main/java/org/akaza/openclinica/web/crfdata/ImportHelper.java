...
public class ImportHelper {
	 ...
	 * @return The DisplayItemBean which is validated.
	 */
	public DisplayItemBean validateDisplayItemBeanText(DiscrepancyValidator discValidator, DisplayItemBean dib,
			String inputName) {
		...
		if (!isNull) {
			if (StringUtil.isBlank(idb.getValue())) {
				...
				if (idt.equals(ItemDataType.ST)) {
					...
					// than
					// the db field length
					discValidator.addValidation(inputName, Validator.LENGTH_NUMERIC_COMPARISON,
					...
					discValidator.addValidation(inputName, Validator.IN_RESPONSE_SET_SINGLE_VALUE, dib.getMetadata()
							.getResponseSet());
				}
