...
<c:if test="${study.studyParameterConfig.discrepancyManagement=='true'}">
  ...
  <c:when test="${originJSP eq 'administrativeEditing'}">
   ...
   <a tabindex="<c:out value="${tabNum + 1000}"/>" href="#" onClick=
    "openDNoteWindow('<c:out value="${contextPath}" />/ViewDiscrepancyNote?subjectId=<c:out value="${studySubject.id}" />&itemId=<c:out value="${itemId}" />&id=<c:out value="${displayItem.data.id}"/>&name=itemData&field=<c:out value="${parsedInputName}"/>&column=value&enterData=1&writeToDB=1','spanAlert-<c:out value="${parsedInputName}"/>','<c:out value="${errorTxtMessage}"/>'); return false;"
