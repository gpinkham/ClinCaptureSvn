...
public class Validator {
    ...
    protected void addError(String fieldName, Validation v) {
        ...
        if (v.isErrorMessageSet()) {
            ...
            case IS_A_PASSWORD:
                // clinovo - tiket #120
                errorMessage = resexception.getString("password_must_be_at_least") + getPwdMinLen(request) + " " + resword.getString("characters_long") + ".";
                break;
            case IS_A_USERNAME:
    ...
    protected HashMap validate(String fieldName, Validation v) {
        ...
        case IS_A_PASSWORD:
            ...
            if (pwdMinLen > 0 && !lengthComparesToStaticValue(fieldName, NumericComparisonOperator.GREATER_THAN_OR_EQUAL_TO, pwdMinLen)) {
                addError(fieldName, v);
            }
            break;
        case IS_A_USERNAME:
    ...
    private int getPwdMinLen(HttpServletRequest request) {
        ConfigurationDao configurationDao = SpringServletAccess.getApplicationContext(request.getSession().getServletContext()).getBean(ConfigurationDao.class);
