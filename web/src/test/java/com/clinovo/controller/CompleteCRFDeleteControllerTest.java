...
public class CompleteCRFDeleteControllerTest extends BaseControllerTest {
	...
	public void testThatCompleteCRFDeleteReturnsCode200() throws Exception {
		...
		this.mockMvc.perform(get(COMPLETE_CRF_DELETE).param("crfId", "1").sessionAttr(SpringController.USER_ROLE, userRole)).andExpect(status().isOk());
	}
	...
	public void testThatCompleteCRFDeleteReturnsCorrectUrl() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.get(COMPLETE_CRF_DELETE).param("crfId", "1").sessionAttr(SpringController.USER_ROLE, userRole))
