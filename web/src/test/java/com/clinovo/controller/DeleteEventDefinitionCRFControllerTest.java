...
public class DeleteEventDefinitionCRFControllerTest extends BaseControllerTest {
	...
	public void testThatMainGetReturnsCorrectStatus() throws Exception {
		mockMvc.perform(get(DELETE_EVENT_DEFINITION_CRF_CONTROLLER).param("id", "1")
				.param("edId", "1")).andExpect(status().isOk());
	}

	@Test
	public void testThatMainGetReturnsCorrectNumberOfAttributes() throws Exception {
		mockMvc.perform(get(DELETE_EVENT_DEFINITION_CRF_CONTROLLER).param("id", "1")
				.param("edId", "1")).andExpect(MockMvcResultMatchers.model()
				.attributeExists("canBeDeleted", "eventCRFs", "edc", "edId", "ruleSetRules"));
	}

	@Test
	public void testThatMainGetReturnsCorrectPageURL() throws Exception {
		mockMvc.perform(get(DELETE_EVENT_DEFINITION_CRF_CONTROLLER).param("id", "1")
				.param("edId", "1")).andExpect(MockMvcResultMatchers.view().name("managestudy/deleteEventDefinitionCrf"));
	}
	...
	public void testThatSubmitReturnsCorrectPageURL() throws Exception {
		mockMvc.perform(get(DELETE_EVENT_DEFINITION_CRF_CONTROLLER).param("edId", "1").param("submit", "true")
				.param("id", "1")).andExpect(MockMvcResultMatchers.view().name("redirect:/InitUpdateEventDefinition?id=1"));
	}

	@Test
	public void testThatSubmitDoesNotReturnsAnyModelAttributes() throws Exception {
		mockMvc.perform(get(DELETE_EVENT_DEFINITION_CRF_CONTROLLER).param("edId", "1").param("submit", "true")
				.param("id", "1")).andExpect(MockMvcResultMatchers.model().size(0));
	}

	@Test
	public void testThatCorrectMessageIsWrittenToSessionIfCRFCannotBeDeleted() throws Exception {
		String expectedMessage = messageSource.getMessage("started_event_crf_found", null, LocaleResolver.getLocale());
		...
		mockMvc.perform(get(DELETE_EVENT_DEFINITION_CRF_CONTROLLER).param("edId", "1").param("submit", "true")
				.param("id", "1")).andExpect(MockMvcResultMatchers.request().sessionAttribute(BaseController.STORED_ATTRIBUTES, storedAttributes));
	}

	@Test
	public void testThatCorrectMessageIsWrittenToSessionIfCRFCanBeDeleted() throws Exception {
		String expectedMessage = messageSource.getMessage("crf_was_deleted_from_event_definition", new String[] {"", "ED-1-NonRepeating"}, LocaleResolver.getLocale());
		HashMap<String, Object> storedAttributes = new HashMap<String, Object>();
		ArrayList<String> pageMessages = new ArrayList<String>();
		pageMessages.add(expectedMessage);
		storedAttributes.put(BaseController.PAGE_MESSAGE, pageMessages);
		mockMvc.perform(get(DELETE_EVENT_DEFINITION_CRF_CONTROLLER).param("edId", "1").param("submit", "true")
				.param("id", "12")).andExpect(MockMvcResultMatchers.request().sessionAttribute(BaseController.STORED_ATTRIBUTES, storedAttributes));
	}
}
