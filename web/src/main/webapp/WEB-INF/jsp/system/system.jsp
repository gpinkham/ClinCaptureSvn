...
<script>
    ...
    function doSubmit() {
        ...
        if (newFormState != firstFormState || $("#formWithStateFlag").val().toLowerCase() == "changed") {
    ...
    function goBack() {
        if ($("#dataChanged").val() == "true") {
            $("#formWithStateFlag").val("changed");
        }
        formWithStateGoBackSmart('<fmt:message key="you_have_unsaved_data3" bundle="${resword}"/>', '${navigationURL}', '${defaultURL}');
    }
...
<form:form id="systemForm" method="post" commandName="systemCommand" enctype="multipart/form-data">
    ...
    <form:hidden path="dataChanged" id="dataChanged"/>
    <div class="sysProps">
                        ...
                        <div id="div_group_id_${grp.group.id}" class="propertyHolder ${grp.opened ? "" : "hidden"}">
                            ...
                            <div id="div_sub_group_id_${grp.group.id}" class="subGroup ${grp.opened ? "" : "hidden"}">
                                <c:forEach items="${grp.subGroups}" var="subGrp" varStatus="subGroupStatus">
                                    ...
                                    <%@include file="systemGroup.jsp" %>
                                    <div id="div_group_id_${subGrp.group.id}"
                                         class="propertyHolder ${subGrp.opened ? "" : "hidden"}">
                                        <%@include file="systemProperty.jsp" %>
                                    </div>
                                </c:forEach>
                            </div>
