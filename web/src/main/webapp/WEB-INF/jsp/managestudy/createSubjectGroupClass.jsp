...
<table border="0" cellpadding="0" cellspacing="0">
  ...
  <tr valign="top"><td class="formlabel"><fmt:message key="type" bundle="${resword}"/>:</td><td><div class="formfieldL_BG">
   ...
   <select name="groupClassTypeId" onChange="javascript:setImageWithTitle('DataStatus_bottom','images/icon_UnsavedData.gif', 'Data has been entered, but not saved.');" class="formfieldL">
      <option value="">--</option>
  ...
  <table border="0" cellpadding="0" cellspacing="0">  
   ...
   <tr>  
    ...
    <td>
     <div class="formfieldS_BG"><input type="text" name="studyGroup<c:out value="${count}"/>" onChange="javascript:setImageWithTitle('DataStatus_bottom','images/icon_UnsavedData.gif', 'Data has been entered, but not saved.');" value="<c:out value="${studyGroup.name}"/>" class="formfieldS"></div>
    </td>  
     
    <td> 
     <div class="formfieldL_BG"><input type="text" name="studyGroupDescription<c:out value="${count}"/>" onChange="javascript:setImageWithTitle('DataStatus_bottom','images/icon_UnsavedData.gif', 'Data has been entered, but not saved.');" value="<c:out value="${studyGroup.description}"/>" class="formfieldL"></div>
    </td>
   </tr> 
   <c:set var="count" value="${count+1}"/>
   </c:forEach>
   <c:if test="${count < 49}">
   <!-- clinovo test for extensible study groups -->
   
    <c:forEach begin="${count}" end="49">
      <tr>  
      <td valign="top"><c:out value="${count+1}"/>&nbsp;</td>    
       <td>
        <div class="formfieldS_BG"><input type="text" name="studyGroup<c:out value="${count}"/>" value="" class="formfieldS" onChange="javascript:setImageWithTitle('DataStatus_bottom','images/icon_UnsavedData.gif', 'Data has been entered, but not saved.');"></div>
       </td>       
       <td> 
       <div class="formfieldL_BG"><input type="text" name="studyGroupDescription<c:out value="${count}"/>" onChange="javascript:setImageWithTitle('DataStatus_bottom','images/icon_UnsavedData.gif', 'Data has been entered, but not saved.');" value="<c:out value="${studyGroup.description}"/>" class="formfieldL"></div>
       </td>
...
<tr>
<td>
  <input type="button" name="BTN_Back" id="GoToSubjectMatrix" title="<fmt:message key="back" bundle="${resword}"/>" value="<fmt:message key="back" bundle="${resword}"/>" class="button_medium" onClick="javascript: return checkGoToEntryStatus('DataStatus_bottom', '<fmt:message key="you_have_unsaved_data2" bundle="${resword}"/>', 'ListSubjectGroupClass');"/>
