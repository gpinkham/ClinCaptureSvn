...
<h1>
	<span class="first_level_header">
		<fmt:message key="restore_event_for_subject" bundle="${resword}"/>: <c:out value="${studySub.label}"/>
	</span>
</h1>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  ...
  <tr valign="top">
	  ...
	  <td class="table_cell">
		  <cc-fmt:formatDate value="${displayEvent.studyEvent.dateStarted}" pattern="${dteFormat}" dateTimeZone="${userBean.userTimeZoneId}"/>
	  </td>
  </tr>
  <tr valign="top">
	  <td class="table_header_column">
	  ...
	  <td class="table_cell">
		  <cc-fmt:formatDate value="${displayEvent.studyEvent.dateEnded}" pattern="${dteFormat}" dateTimeZone="${userBean.userTimeZoneId}"/>
	  </td>
  </tr>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
 ...
 <c:forEach var="dec" items="${displayEvent.displayEventCRFs}">
	<tr>
		...
		<td class="table_cell"><cc-fmt:formatDate value="${dec.eventCRF.dateInterviewed}" pattern="${dteFormat}" dateTimeZone="${userBean.userTimeZoneId}"/></td>
   ...
   <c:choose>
    <c:when test="${!empty displayEvent.displayEventCRFs}">
     <form action='RestoreStudyEvent?action=submit&id=<c:out value="${displayEvent.studyEvent.id}"/>&studySubId=<c:out value="${studySub.id}"/>' method="POST">
      ...
      &nbsp;
      <input type="submit" name="submit" value="<fmt:message key="submit" bundle="${resword}"/>" class="button_medium medium_submit" onClick='return confirmSubmit({ message: "<fmt:message key="this_event_has_CRF_data_shown_restore" bundle="${resword}"/>", height: 150, width: 500, submit: this });'>
     </form>
