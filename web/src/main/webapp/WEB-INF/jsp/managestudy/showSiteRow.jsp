      ...
      <c:if test="${userRoleId eq 1 or userRoleId eq 2}">
          <c:choose>
              <c:when test="${currRow.bean.showUnlockEventsButton}">
                  <td>
                      <a href="LockSite?id=${currRow.bean.id}&action=unlock"><img src="images/bt__Unlock.png" border="0" align="left" alt="<fmt:message key="unlockSiteStudySubjects" bundle="${resword}"/>" title="<fmt:message key="unlockSiteStudySubjects" bundle="${resword}"/>" hspace="4"/></a>
                  </td>
              </c:when>
              <c:when test="${currRow.bean.showLockEventsButton}">
                  <td>
                      <a href="LockSite?id=${currRow.bean.id}&action=lock"><img src="images/bt__Lock.png" border="0" align="left" alt="<fmt:message key="lockSiteStudySubjects" bundle="${resword}"/>" title="<fmt:message key="lockSiteStudySubjects" bundle="${resword}"/>" hspace="4"/></a>
                  </td>
              </c:when>
