...
<h1>
	<span class="first_level_header">
		<fmt:message key="confirm_restore_of_CRF" bundle="${resword}"/>
	</span>
</h1>
...
<div class="textbox_center_admin">
	<table class="table_vertical">
		<tr>
			...
			<td><c:out value="${crfToRestore.name}"/></td>
		</tr>
		<tr>
			<td><fmt:message key="description" bundle="${resword}"/>:</td>
			<td><c:out value="${crfToRestore.description}"/></td>
		</tr>
	</table>
</div>
...
<form action='RestoreCRF?action=submit&id=<c:out value="${crfToRestore.id}"/>' method="POST">
	...
	<input type="submit" name="submit" value="<fmt:message key="submit" bundle="${resword}"/>" class="button_medium medium_submit" onClick='return confirmSubmit({ message: "<fmt:message key="if_you_restore_this_CRF" bundle="${restext}"/>", height: 150, width: 500, submit: this });'>
