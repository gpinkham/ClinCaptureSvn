...
<tr valign="top" bgcolor="#F5F5F5">
	...
	<td class="table_cell">
	 ...
	 <tr class="innerTable">
	 	<td><a href="ViewSingleJob?tname=<c:out value="${currRow.bean.fullName}" />&gname=<c:out value="${currRow.bean.groupName}" />" data-cc-exportJobId="${currRow.bean.fullName}" onclick="setAccessedObjected(this)"><img name="bt_View1" src="images/bt_View.gif" border="0" alt="<fmt:message key="view" bundle="${resword}"/>" title="<fmt:message key="view" bundle="${resword}"/>" align="left" hspace="2"></a></td>
		<td><a href="UpdateJobExport?tname=<c:out value="${currRow.bean.fullName}" />" onclick="setAccessedObjected(this)"><img name="bt_Edit1" src="images/bt_Edit.gif" border="0" alt="<fmt:message key="edit" bundle="${resword}"/>" title="<fmt:message key="edit" bundle="${resword}"/>" align="left" hspace="2"></a></td>
		<td>
		<c:choose>
			...
			<c:otherwise>
				<a href="PauseJob?tname=<c:out value="${currRow.bean.fullName}" />&gname=0" onClick='return confirmDialog({ message: "<fmt:message key="confirm_restoring_this_job" bundle="${restext}"/>", height: 150, width: 500, aLink: this, highlightRow: true });'><img border="0" title="<fmt:message key="restore" bundle="${resword}"/>" src="images/bt_Restore.gif" alt="<fmt:message key="restore" bundle="${resword}"/>" align="left" hspace="2"/></a>
			</c:otherwise>
		</c:choose>
		</td>&nbsp;
		<c:if test="${userBean.sysAdmin}">
			<td>
				<a href="PauseJob?tname=<c:out value="${currRow.bean.fullName}" />&del=y&gname=0" onClick='return confirmDialog({ message: "<fmt:message key="confirm_deleting_this_job" bundle="${restext}"/>", height: 150, width: 500, aLink: this, highlightRow: true });'><img border="0" title="<fmt:message key="delete" bundle="${resword}"/>" src="images/bt_Delete.gif" alt="<fmt:message key="delete" bundle="${resword}"/>" align="left" hspace="2"/></a>
			</td>
		</c:if>
	 </tr>
		</table>
