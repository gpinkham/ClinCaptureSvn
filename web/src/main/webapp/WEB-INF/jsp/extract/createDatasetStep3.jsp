...
<c:choose>
	...
	<c:otherwise>
		<h1>
			<span class="first_level_header">
				<fmt:message key="create_dataset" bundle="${resword}"/>: <fmt:message key="define_temporal_scope" bundle="${resword}"/> 
				<a href="javascript:openDocWindow('help/4_2_createDataset_Help.html#step2')">
...
<table>
  <tr>
        <td>
            <input type="button" name="BTN_Back" id="PreviousPage" value="<fmt:message key="back" bundle="${resword}"/>" class="button_medium medium_back" size="50" onclick="datasetConfirmBack('<fmt:message key="you_have_unsaved_data2" bundle="${resword}"/>', 'datasetForm', 'CreateDataset', 'back_to_viewselected');"/></td>
        <td>
			<input id="btnSubmit" type="submit" name="submit" value="<fmt:message key="continue" bundle="${resword}"/>" class="button_medium medium_continue"/></td>
        <td>
