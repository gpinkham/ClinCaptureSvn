...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<script language="JavaScript" type="text/JavaScript">
		...
		jjq('.jmesa').find("tr[class*='header']").find('td').css(
				'background-color', '#AA62C6');
		jjq('.sidebar_tab_content').find('span').css('color', '#AA62C6');

		jjq('.tab_R_h').find('span').css('color', '#AA62C6');
		jjq('.jmesa').find("tr[class*='header']").find('td').css('color',
				'#FFFFFF');
		jjq('.aka_revised_content').find('span.title_submit').css('color',
				'#D4A718');
		...
		jjq("input[src*='images/icon_DoubleCheck_Action.gif']").each(
				function() {
					var newSrc = jjq(this).attr('src');
					newSrc = newSrc.replace('images/', 'images/violet/');
					jjq(this).attr('src', newSrc);
		...
		jjq('input.navSearchButton:submit').css('background-image',
				'url(images/violet/bt_navSearch.gif)');
		jjq('input.button_search:submit').css('background-image',
				'url(images/violet/button_Search_BG.gif)');
		jjq('button.button_search').css('background-image',
				'url(images/violet/button_Search_BG.gif)');
	</script>
     </c:if>
