            ...
            <td style="padding-top: 3px; padding-left: 6px;" nowrap>
                <div class="moreCrfInfoBlock">
                    <div class="crfInfoBlock">
                        ...
                        <div class="crfInfoDiv"><b>Subject: </b>${studySubject.label}</div>
                        <div class="crfInfoDiv"><b><fmt:message key="event" bundle="${resword}"/>: </b><c:out value="${toc.studyEventDefinition.name}" />&nbsp;(<fmt:formatDate
                                value="${toc.studyEvent.dateStarted}" pattern="${dteFormat}" />)</div>
                    </div>
                    <div class="crfInfoBlock">
        ...
        <tr id="CRF_infobox_closed">
            <td style="padding-top: 3px; padding-left: 6px;" nowrap>
                <div class="moreCrfInfoBlock">
                    <div class="crfInfoBlock">
                        <div class="crfInfoDiv"><b>Study: </b>${studyTitle}</div>
                        <div class="crfInfoDiv"><b>Subject: </b>${studySubject.label}</div></div>
                        <div class="crfInfoDiv"><b><fmt:message key="event" bundle="${resword}"/>: </b><c:out value="${toc.studyEventDefinition.name}" />&nbsp;(<fmt:formatDate
                                value="${toc.studyEvent.dateStarted}" pattern="${dteFormat}" />)</div>
                    </div>
                    <div class="crfInfoBlock">
...
<c:if test="${study ne null}">
    ...
    <!-- secondary label viewable? <c:out value="${study.studyParameterConfig.secondaryLabelViewable}" /> -->
...
<!-- added clinovo # not defined, 11/02/2012 -->
<tr>
    <c:choose>
        <c:when test="${secondaryLabelViewable and secondaryIdShow}">
            <td class="table_cell_noborder">
                <b>${secondaryIdLabel}:</b><br />
            </td>
            <td>
                <div id=ssIdLabel><c:out value="${studySubject.secondaryLabel}" /><br /></div>
            </td>
        </c:when>
        <c:otherwise>
                ...
                <c:choose>
                     ...
                     <c:otherwise>
                        <a id="nameNote1" href="#" onmouseout="UnTip();" onmouseover="callTip(genToolTipFromArray('interviewNotes'));"
                           onClick="openDSNoteWindow('<c:out value="${contextPath}" />/CreateDiscrepancyNote?subjectId=${studySubject.id}&viewData=y&id=<c:out value="${toc.eventCRF.id}"/>&name=eventCrf&field=interviewer&column=interviewer_name&writeToDB=1&new=${isNewDN}','spanAlert-interviewer', event); return false;">
                            ...
                            <input type="hidden" value="<c:out value="${contextPath}" />/ViewDiscrepancyNote?writeToDB=1&subjectId=${studySubject.id}&itemId=${itemId}&id=<c:out value="${toc.eventCRF.id}"/>&name=eventCrf&field=interviewer&column=interviewer_name&enterData=${enterData}&monitor=${monitor}&blank=${blank}"/>
                        </a>
                     </c:otherwise>
                 </c:choose>
                        <%--</c:if>--%>
                ...
                <c:choose>
                    <c:when test="${hasDateNote eq 'yes'}">
                        <a href="#"  onmouseover="callTip(genToolTipFromArray('dateNotes') );" onmouseout="UnTip();"
                           onClick="openDNoteWindow('<c:out value="${contextPath}" />/ViewDiscrepancyNote?writeToDB=1&subjectId=${studySubject.id}&itemId=${itemId}&id=${InterviewerDateNote.eventCRFId}&name=${InterviewerDateNote.entityType}&field=interviewDate&column=${InterviewerDateNote.column}&enterData=${enterData}&monitor=${monitor}&blank=${blank}','spanAlert-interviewDate'); return false;">
                    ...
                    <c:otherwise>
                        <a href="#"  onmouseover="callTip(genToolTipFromArray('dateNotes') );" onmouseout="UnTip();"
                           onClick="openDNoteWindow('<c:out value="${contextPath}" />/CreateDiscrepancyNote?subjectId=${studySubject.id}&id=<c:out value="${toc.eventCRF.id}"/>&name=eventCrf&field=interviewDate&column=date_interviewed&writeToDB=1&new=${isNewDNDate}','spanAlert-interviewDate', undefined, event); return false;">
                            ...
                            <input type="hidden" value="<c:out value="${contextPath}" />/ViewDiscrepancyNote?writeToDB=1&subjectId=${studySubject.id}&itemId=${itemId}&id=<c:out value="${toc.eventCRF.id}"/>&name=eventCrf&field=interviewDate&column=date_interviewed&enterData=${enterData}&monitor=${monitor}&blank=${blank}"/>
                        </a>
                    </c:otherwise>
                  </c:choose>
                        <%--  </c:if>--%>
