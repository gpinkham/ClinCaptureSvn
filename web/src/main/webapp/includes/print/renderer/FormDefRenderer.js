function FormDefRenderer(json, studyEventOID, studyEventRepeatKey) {
    ...
    this.renderPrintableForm = function () {
        return RenderUtil.render(RenderUtil.get("print_form_def"), {name: this.name, eventCRFLink: this.eventCRFLink});
    }
    this.renderInteractiveForm = function () {
        return RenderUtil.render(RenderUtil.get("e_form_def"), {name: this.name, eventCRFLink: this.eventCRFLink});
    }
    ...
    this.renderDiscrepancyNotes = function (discrepancyNotes, attribute) {
        ...
        var s = RenderUtil.render(RenderUtil.get(template), {
            ...
            title: attribute + ": " + app_nav_notes_and_discrepanciesLabel
        });
    ...
    this.renderAuditLogs = function (auditLogs) {
        ...
        var s = RenderUtil.render(RenderUtil.get(template), {
            auditLogs: auditLogs,
            title: app_eventCRFLabel + " " + app_Audit_HistoryLabel
        });
        return s[0].outerHTML;
    }
}
