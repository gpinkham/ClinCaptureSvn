		...
		.extend(
				...
				(function() {
					...
					function extractScripts() {
						...
						return (this.match(matchAll) || []).map(function(
								scriptTag) {
...
(function(GLOBAL) {
	...
	function wrap(element, wrapper, attributes) {
		...
		if (Object.isElement(wrapper)) {
			prt$(wrapper).writeAttribute(attributes || {});
	...
	function empty(element) {
		return prt$(element).innerHTML.blank();
	}
	...
	function firstDescendant(element) {
		element = prt$(element).firstChild;
	...
	function immediateDescendants(element) {
		var results = [], child = prt$(element).firstChild;
	...
	function _recursivelyFind(element, property, expression, index) {
		element = prt$(element), expression = expression || 0,
				index = index || 0;
	...
	function readAttribute(element, name) {
		return prt$(element).getAttribute(name);
	}
	...
	function hasAttribute(element, attribute) {
		...
		var node = prt$(element).getAttributeNode(attribute);
	...
	function _getFlag(element, attribute) {
		return prt$(element).hasAttribute(attribute) ? attribute : null;
	}
	...
	function setOpacity_IE(element, value) {
		...
		element = hasLayout_IE(prt$(element));
...
(function() {
	...
	function getPixelValue(value, property, context) {
		...
		if (element && isPercentage) {
			...
			if (context === document.viewport) {
				...
				if (isHorizontal) {
					whole = prt$(context).measure('width');
				} else if (isVertical) {
					whole = prt$(context).measure('height');
				}
			}
	...
	function isDisplayed(element) {
		while (element && element.parentNode) {
			...
			element = prt$(element.parentNode);
		}
		return true;
	}
					...
					Hash,
					{
						...
						_begin : function() {
							...
							if (width && (positionedWidth === width)) {
								...
								var parent = element.parentNode, pLayout = prt$(
										parent).getLayout();
	...
	function measure(element, property) {
		return prt$(element).getLayout().get(property);
	}
	...
	function getOffsetParent(element) {
		...
		if (isDocument(element) || isDetached(element) || isBody(element)
				|| isHtml(element))
			return prt$(document.body);

		var isInline = (Element.getStyle(element, 'display') === 'inline');
		if (!isInline && element.offsetParent)
			return prt$(element.offsetParent);

		while ((element = element.parentNode) && element !== document.body) {
			if (Element.getStyle(element, 'position') !== 'static') {
				return isHtml(element) ? prt$(document.body) : prt$(element);
			}
		}
...
Form.Methods = {
	...
	getElements : function(form) {
		var elements = prt$(form).getElementsByTagName('*');
	...
	findFirstElement : function(form) {
		var elements = prt$(form).getElements().findAll(function(element) {
	...
	request : function(form, options) {
		form = prt$(form), options = Object.clone(options || {});
...
(function(GLOBAL) {
	...
	function fire(element, eventName, memo, bubble) {
		element = getFireTarget(prt$(element));
	...
	document.getElementsByClassName = function(instanceMethods) {
		...
		instanceMethods.getElementsByClassName = Prototype.BrowserFeatures.XPath
				? function(element, className) {
					...
					var cond = /\s/.test(className) ? $w(className).map(iter)
							.join('') : iter(className);
				...
				: function(element, className) {
					...
					var nodes = prt$(element).getElementsByTagName('*');
					className = ' ' + className + ' ';
		...
		return function(className, parentElement) {
			return prt$(parentElement || document.body).getElementsByClassName(
					className);
		};
	}(Element.Methods);
