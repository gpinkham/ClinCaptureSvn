PLEASE NOTE This refers to users who are upgrading from OpenClinica 3.0.x to ClinCapture 1.0.2 or later.  We are preserving these notes in their entirety.
...
If you do not run this script and remove the duplicate rows, you may not be able to upgrade to future versions of ClinCapture.
...
EXAMPLE: /opt/PostgreSQL/8.4/bin/psql -u clincapture -d DBNAME < Duplicate_Report.sql
Replace DBNAME in the above example with the name of your database.  Also note that we are accessing the database as the user 'clincapture'.
...
EXAMPLE: /opt/PostgreSQL/8.4/bin/psql -u clincapture -d DBNAME < Duplicate_Delete_v2.sql
...
EXAMPLE: /opt/PostgreSQL/8.4/bin/psql -u clincapture -d DBNAME < Manual_Duplicate_Delete.sql
Replace DBNAME in the above example with the name of your database.
