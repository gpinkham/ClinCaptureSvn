...
public class CRFMaskingController extends BaseController {
	...
	public String openCRFsMaskingPage(Model model, HttpServletRequest request) {
		...
		HashMap<String, ArrayList<StudyBean>> sitesByStudies = getSitesByStudyMap(sBeans, user);
			...
			if (paramName.contains(ITEM_PREFIX) && paramName.contains(CRF_PREFIX) && paramName.contains(EVENT_PREFIX)
				...
				if (request.getParameter(paramName).equals("masked")) {
					...
					if (sur.isActive()) {
						...
						mask.setStudyUserRoleId(sur.getId());
	 ...
	 */
	private HashMap<String, ArrayList<StudyBean>> getSitesByStudyMap(ArrayList<StudyBean> sBeans,
																	 UserAccountBean user) {
		HashMap<String, ArrayList<StudyBean>> sitesByStudies = new HashMap<String, ArrayList<StudyBean>>();
