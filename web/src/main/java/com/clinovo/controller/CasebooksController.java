...
public class CasebooksController extends SpringController {
	...
	public String changeDefinitionOrdinalHandler(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		boolean secondaryIdRequired = !studyParameterValueDao
				.findByHandleAndStudy(studyBean.getId(), "secondaryIdRequired").getValue().equalsIgnoreCase("not_used")
				&& !studyParameterValueDao.findByHandleAndStudy(studyBean.getId(), "secondaryIdRequired").getValue()
						.isEmpty();
		boolean collectDob = !studyParameterValueDao.findByHandleAndStudy(studyBean.getId(), "collectDob").getValue()
				.equalsIgnoreCase("3")
				&& !studyParameterValueDao.findByHandleAndStudy(studyBean.getId(), "collectDob").getValue().isEmpty();
		boolean genderRequired = studyParameterValueDao.findByHandleAndStudy(studyBean.getId(), "genderRequired")
	...
	public void generateSiteCasebooks(HttpServletRequest request) throws Exception {
		...
		if (siteId != null) {
			...
			for (StudySubjectBean studySubjectBean : studySubjectBeans) {
				String reportHTML = getSubjectCasebookHTML(request, true, true, studySubjectBean.getOid());
				OutputStream os = new FileOutputStream(
						getFile(studySubjectBean.getOid(), DIR_NAME + File.separator + parentStudyBean.getOid()));
				HtmlToPdfUtil.buildPdf(reportHTML, os);
			}
		}
	}
	...
	public void generateCasebook(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String reportHTML = getSubjectCasebookHTML(request, audit, notes, subjectOid);
		OutputStream os = new FileOutputStream(getFile(subjectOid, DIR_NAME + File.separator + studyBean.getOid()));
		HtmlToPdfUtil.buildPdf(reportHTML, os);
		HtmlToPdfUtil.buildPdf(reportHTML, response.getOutputStream());
	}
	...
	public String generateCasebooks(HttpServletRequest request) throws Exception {
		...
		StudyBean parentStudyBean = studyBean.isSite()
				? (StudyBean) studyDAO.findByPK(studyBean.getParentStudyId())
				: studyBean;
		for (String ssOid : studySubjectOidList) {
			String reportHTML = getSubjectCasebookHTML(request, true, true, ssOid);
			OutputStream os = new FileOutputStream(
					getFile(ssOid, DIR_NAME + File.separator + parentStudyBean.getOid()));
			HtmlToPdfUtil.buildPdf(reportHTML, os);
		}
		...
		String message = messageSource.getMessage("casebooks_generated", null, LocaleResolver.getLocale())
				.replace("{0}", request.getContextPath() + "/pages");
	...
	private String getSubjectCasebookHTML(HttpServletRequest request, boolean includeAudit, boolean includeNotes,
			String ssOid) throws IOException {
		...
		StudyBean parentStudyBean = studyBean.isSite()
				? (StudyBean) studyDAO.findByPK(studyBean.getParentStudyId())
				: studyBean;
		return metaDataService.getHTML(request.getContextPath(), currentUser, parentStudyBean.getOid(), ssOid, "*", "*",
				includeNotes, includeAudit, true, locale);
	}
	...
	public String downloadCasebookPageHandler(HttpServletRequest request) {
		...
		List<StudySubjectBean> studySubjects = getStudySubjectBeanList(studySubjectOid, studyBean.getId(),
				studyBean.isSite());

		String sortingColumn = request.getParameter(DownloadCasebooksTableFactory.EBL_SORT_COLUMN);
		DownloadCasebooksTableFactory downloadCasebooksTableFactory = new DownloadCasebooksTableFactory(sortingColumn,
	 ...
	 * @throws IOException
	 *             for all IO exceptions.
	 */
	@RequestMapping("/downloadCasebookFromStorage")
	public String downloadCasebookFromStorage(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
	...
	private String getCasebookFilePath(String studyOid, String studySubjectOid) {
		return SQLInitServlet.getField("filePath") + "print" + File.separator + "Casebooks" + File.separator + studyOid
				+ File.separator + studySubjectOid + ".pdf";
	}
	...
	private List<String> getStudySubjectOidList(String oid) {
		String datasetFilePath = SQLInitServlet.getField("filePath") + "print" + File.separator + "Casebooks"
				+ File.separator + oid;
	...
	private void sendCompletionEmail(HttpServletRequest request, String subjectOID) {
		...
		String message = EmailUtil.getEmailBodyStart() + subject + "<br/><ul>"
				+ messageSource.getMessage("job_error_mail.serverUrl", null, locale) + " "
				+ SQLInitServlet.getSystemURL() + "</li>"
				+ messageSource.getMessage("job_error_mail.studyName", null, locale) + " " + study.getName()
				+ "</li></ul><br/>" + messageSource.getMessage("casebooks_download_link", null, locale) + ": <a href=\""
