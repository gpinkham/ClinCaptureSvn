...
/**
 * Custom tag for building RemoveEventCRF link.
 */
@SuppressWarnings("serial")
public class RemoveEventCRFLinkTag extends TagSupport {

	private static final Logger LOGGER = LoggerFactory.getLogger(RemoveEventCRFLinkTag.class);
	...
	public int doStartTag() throws JspException {
		...
		if (object != null && object instanceof DisplayEventCRFBean && !subjectId.isEmpty()) {
			...
			if (!crfMaskingService.isEventDefinitionCRFMasked(edcBean.getId(), user.getId(), edcBean.getStudyId())) {
				link = "<a href=\"RemoveEventCRF?action=confirm&id=" + dec.getEventCRF().getId() + "&studySubId="
						+ subjectId + "\" "
						+ "onMouseDown=\"javascript:setImage('bt_Remove1','images/bt_Remove_d.gif');\" "
						+ "onMouseUp=\"javascript:setImage('bt_Remove1','images/bt_Remove.gif');\" " + "onclick=\""
						+ onClick + "\">" + "<img name=\"bt_Remove1\" " + "src=\"images/bt_Remove.gif\" border=\"0\""
						+ "alt=\"" + messageSource.getMessage("remove", null, locale) + "\" " + "title=\""
						+ messageSource.getMessage("remove", null, locale) + "\" " + "align=\"left\" " + "hspace=\""
						+ hspace + "\"></a>";
			}
		}
