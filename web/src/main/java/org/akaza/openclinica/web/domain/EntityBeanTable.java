 ...
 * 
 * <p>
 * This class facilitates the display of multiple rows with the following UI features:
 * <ul>
 ...
 * 
 * <li>In the servlet, obtain a new EntityBeanTable from the FormProcessor: <br/>
 * <code>EntityBeanTable table = fp.getEntityBeanTable();</code> <br/>
 * Note that this step covers both the case where the table is being generated for the first time, and the case where
 * the table has already been displayed, and the user has requested a modification to the display (e.g., go to the next
 * page or sort by a specific column) - the FormProcessor automatically reads in any requested modifications from the
 * request and applies them to the table.
 * 
 ...
 * </code>
 * 
 * <li>Populate the table with the columns you wish to display: <br/>
 * <code>
 * String columns[] = { "username", "first name", "last name", "status", "actions"};
 * table.setColumns(new ArrayList(Arrays.asList(columns)));
 * </code>
 * 
 * <li>Populate the table with the base query which invokes the screen you're about to display: <code>
 ...
 * <code>setTable(table);
 * <br/>This method is inherited from SpringServlet.
 * 
 ...
 * 
 *      <p>Method 'computeDisplay()' modified by ywang to remove duplicated items when search by keywords.
 */
...
public class EntityBeanTable {
	...
	/**
	 * All of the rows which might be displyaed, before computeDisplay() is called. All of the rows which will be
	 * displayed (after applying the tabling parameters), after computeDisplay() is called. Each element is an <a
	 * href="{@docRoot} /org/akaza/openclinica/core/EntityBeanRow.html">EntityBeanRow</a>s
	 */
	...
	/**
	 * How many page numbers are there total,always equal to ceil(rows.size() / NUM_ROWS_PER_PAGE); maintained by
	 * setRows.
	 */
	protected int totalPageNumbers;

	/**
	 * Indicates the column we're currently sorting by. Ranges form 0 to columns.size() - 1; interpreted as an index
	 * into columns.
	 */
	protected int sortingColumnInd;

	/**
	 * Indicates whether the sorting column was explicitly set in the GET or POST variables. This makes the
	 * functionality in setSortingIfNotExplicitlySet possible.
	 */
	protected boolean sortingColumnExplicitlySet = false;

	/**
	 * <code>true</code> if we're sorting in ascending order, <code>false</code> otherwise
	 */
	protected boolean ascendingSort;

	/**
	 * <code>true</code> if we use the keyword filter <code>false</code> otherwise
	 */
	...
	/**
	 * <code>true</code> if we are viewing at most NUM_ROWS_PER_PAGE at a time <code>false</code> if we are viewing all
	 * of the pages on one screen
	 */
	protected boolean paginated = true;

	/**
	 * A set of links to display in the upper-right hand corner. Each element is a Link object.
	 */
	...
	/**
	 * @param columns
	 *            The columns to set. Each element is a String with the column name.
	 */
	...
	/**
	 * Re-computes the correct value of page numbers as: <code>rows.size() / NUM_ROWS_PER_PAGE</code>
	 */
	...
	/**
	 * Compute the subset of rows which should be shown on the screen. Note that the tabling parameters should be set
	 * properly before this method is called!
	 */
	...
	/**
	 * Signal that the column at index <code>i</code> should not be displayed with a link in the JSP; this prevents
	 * users from sorting on the i-th column.
	 * 
	 * @param i
	 *            The index of the column whose link should not be displayed. The first column is 0.
	 */
