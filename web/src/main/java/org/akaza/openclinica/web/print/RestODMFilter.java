...
public class RestODMFilter extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        UserAccountBean userAccountBean = (UserAccountBean) request.getSession().getAttribute(BaseController.USER_BEAN_NAME);
        ...
        if (currentUserRole.getId() == 0) {
            request.getSession().setAttribute("casebook_exception", respage.getString("does_not_have_access_for_subject_casebook"));
