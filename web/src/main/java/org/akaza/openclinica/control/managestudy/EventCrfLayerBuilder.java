...
public class EventCrfLayerBuilder {
	...
	public EventCrfLayerBuilder(SubjectBean subject, Integer rowCount, List<StudyEventBean> studyEvents,
			DataEntryStage eventCrfStatus, EventCRFBean eventCrfBean, StudySubjectBean studySubject,
			StudyBean currentStudy, StudyUserRoleBean currentRole, UserAccountBean currentUser,
	...
	void buildLock() {
		...
		if (eventCrfStatus == DataEntryStage.ADMINISTRATIVE_EDITING) {
			lockLinkBuilder(html, studySubjectLabel, rowCount, crf, "images/CRF_status_icon_Complete_collapse.gif",
					"images/CRF_status_icon_Complete.gif");
		} else if (eventCrfStatus == DataEntryStage.INITIAL_DATA_ENTRY_COMPLETE) {
			lockLinkBuilder(html, studySubjectLabel, rowCount, crf,
					"images/CRF_status_icon_InitialDEcomplete_collapse.gif",
					"images/CRF_status_icon_InitialDEcomplete.gif");
		} else if (eventCrfStatus == DataEntryStage.DOUBLE_DATA_ENTRY) {
			lockLinkBuilder(html, studySubjectLabel, rowCount, crf, "images/CRF_status_icon_DDE_collapse.gif",
					"images/CRF_status_icon_DDE.gif");
		} else if (eventCrfStatus == DataEntryStage.LOCKED) {
			// Do Nothing
		} else if (eventCrfStatus == DataEntryStage.UNCOMPLETED) {
			lockLinkBuilder(html, studySubjectLabel, rowCount, crf, "images/CRF_status_icon_Scheduled.gif",
					"images/CRF_status_icon_Scheduled.gif");
		} else if (eventCrfStatus == DataEntryStage.INVALID) {
			lockLinkBuilder(html, studySubjectLabel, rowCount, crf, "images/CRF_status_icon_Invalid_collapse.gif",
					"images/CRF_status_icon_Invalid.gif");
		} else {
			lockLinkBuilder(html, studySubjectLabel, rowCount, crf, "images/CRF_status_icon_Started.gif",
					"images/CRF_status_icon_Started.gif");
		}
		html.divEnd();
	...
	void clickToEnterData() {
		...
		String studySubjectLabel = studySubject.getLabel();
		html.tr(0).id("Menu_off_" + studySubjectLabel + "_" + crf.getId() + "_" + rowCount).style("display: all")
				.close();
		...
		if (eventCrfStatus == DataEntryStage.DOUBLE_DATA_ENTRY_COMPLETE
			...
			if (getStudyEvent() != null && !currentRole.isMonitor() && currentStudy.getStatus() == Status.AVAILABLE) {
				if (!hiddenCrf()) {
					html.tr(0).valign("top").close();
					html.td(0).styleClass(table_cell_left).close();
					initialDataEntryLink(html, eventCrfBean == null ? new EventCRFBean() : eventCrfBean, studySubject,
							eventDefinitionCrf, getStudyEvent());
					html.nbsp().nbsp();
					initialDataEntryLink(html, eventCrfBean == null ? new EventCRFBean() : eventCrfBean, studySubject,
							eventDefinitionCrf, getStudyEvent(), reswords.getString("enter_data"));
					html.tdEnd().trEnd(0);
				}
			}
			...
			if (!currentRole.isMonitor() && currentStudy.getStatus() == Status.AVAILABLE) {
				if (eventCrfStatus == DataEntryStage.INITIAL_DATA_ENTRY_COMPLETE
						|| eventCrfStatus == DataEntryStage.DOUBLE_DATA_ENTRY) {
					if (!hiddenCrf()) {
						html.tr(0).valign("top").close();
						html.td(0).styleClass(table_cell_left).close();
	...
	void buildEnd() {
		...
		if (eventCrfStatus == DataEntryStage.DOUBLE_DATA_ENTRY_COMPLETE
				|| eventCrfStatus == DataEntryStage.ADMINISTRATIVE_EDITING) {
			iconLinkBuilder(html, studySubjectLabel, rowCount, crf, "images/CRF_status_icon_Complete_expand.gif",
					"images/CRF_status_icon_Complete.gif");
		} else if (eventCrfStatus == DataEntryStage.INITIAL_DATA_ENTRY_COMPLETE) {
			iconLinkBuilder(html, studySubjectLabel, rowCount, crf,
					"images/CRF_status_icon_InitialDEcomplete_expand.gif",
					"images/CRF_status_icon_InitialDEcomplete.gif");
		} else if (eventCrfStatus == DataEntryStage.LOCKED) {
			// Do Nothing
		} else if (eventCrfStatus == DataEntryStage.UNCOMPLETED) {
			iconLinkBuilder(html, studySubjectLabel, rowCount, crf, "images/CRF_status_icon_Scheduled.gif",
					"images/CRF_status_icon_Scheduled.gif");
		} else if (eventCrfStatus == DataEntryStage.INVALID) {
			iconLinkBuilder(html, studySubjectLabel, rowCount, crf, "images/CRF_status_icon_Invalid_expand.gif",
					"images/CRF_status_icon_Invalid.gif");
		} else {
			iconLinkBuilder(html, studySubjectLabel, rowCount, crf, "images/CRF_status_icon_Started.gif",
					"images/CRF_status_icon_Started.gif");
		}
	}

	private void viewEventCrfContentLink(HtmlBuilder builder, StudySubjectBean studySubject, EventCRFBean eventCrf,
			StudyEventBean studyEvent) {
		String href = "ViewEventCRFContent?id=" + studySubject.getId() + "&ecId=" + eventCrf.getId() + "&eventId="
				+ studyEvent.getId();
	...
	private void viewEventCrfContentLinkPrint(HtmlBuilder builder, StudySubjectBean studySubject,
			EventCRFBean eventCrf, StudyEventBean studyEvent) {
		String href = "ViewEventCRFContent?id=" + studySubject.getId() + "&ecId=" + eventCrf.getId() + "&eventId="
				+ studyEvent.getId();
		builder.a().href(href).close();
	...
	private void viewSectionDataEntryParameterized(HtmlBuilder builder, EventDefinitionCRFBean eventDefinitionCrf,
			String link) {
		String href = "ViewSectionDataEntry?eventDefinitionCRFId=" + eventDefinitionCrf.getId() + "&crfVersionId="
				+ eventDefinitionCrf.getDefaultVersionId() + "&tabId=1" + "&exitTo=ListStudySubjects";
		builder.a().href(href).close();
		builder.append(link);
		builder.aEnd();
	}
	...
	private void initialDataEntryLink(HtmlBuilder builder, EventCRFBean eventCrf, StudySubjectBean studySubject,
			EventDefinitionCRFBean eventDefinitionCrf, StudyEventBean studyEvent) {
		String href = "InitialDataEntry?eventDefinitionCRFId=" + eventDefinitionCrf.getId() + "&studyEventId="
				+ studyEvent.getId() + "&subjectId=" + studySubject.getSubjectId() + "&eventCRFId=" + eventCrf.getId()
				+ "&crfVersionId=" + eventDefinitionCrf.getDefaultVersionId() + "&exitTo=ListStudySubjects";
		builder.a().href(href).close();
		builder.img().src("images/bt_Edit.gif").border("0").align("left").close();
		builder.aEnd();
	}
	...
	private void initialDataEntryLink(HtmlBuilder builder, EventCRFBean eventCrf, StudySubjectBean studySubject,
			EventDefinitionCRFBean eventDefinitionCrf, StudyEventBean studyEvent, String link) {
		String href = "InitialDataEntry?eventDefinitionCRFId=" + eventDefinitionCrf.getId() + "&studyEventId="
				+ studyEvent.getId() + "&subjectId=" + studySubject.getSubjectId() + "&eventCRFId=" + eventCrf.getId()
				+ "&crfVersionId=" + eventDefinitionCrf.getDefaultVersionId() + "&exitTo=ListStudySubjects";
		builder.a().href(href).close();
	...
	private void linkBuilder(HtmlBuilder builder, String studySubjectLabel, Integer rowCount, CRFBean crf, String icon) {
		...
		String onClick3 = "layersShowOrHide('hidden','Lock_" + studySubjectLabel + "_" + crf.getId() + "_" + rowCount
				+ "'); ";
		String onClick4 = "javascript:setImage('CRFicon_" + studySubjectLabel + "_" + crf.getId() + "_" + rowCount
				+ "','" + icon + "'); ";
	...
	private void lockLinkBuilder(HtmlBuilder builder, String studySubjectLabel, Integer rowCount, CRFBean crf,
			String collapsedIcon, String icon) {
		String href1 = "javascript:leftnavExpand('Menu_on_" + studySubjectLabel + "_" + crf.getId() + "_" + rowCount
				+ "'); ";
		String href2 = "javascript:leftnavExpand('Menu_off_" + studySubjectLabel + "_" + crf.getId() + "_" + rowCount
				+ "'); ";
		String onmouseover = "layersShowOrHide('visible','Event_" + studySubjectLabel + "_" + crf.getId() + "_"
				+ rowCount + "'); ";
		onmouseover += "javascript:setImage('CRFicon_" + studySubjectLabel + "_" + crf.getId() + "_" + rowCount + "','"
				+ collapsedIcon + "');";
	...
	private void iconLinkBuilder(HtmlBuilder builder, String studySubjectLabel, Integer rowCount, CRFBean crf,
			String expandedIcon, String icon) {
		String href1 = "javascript:leftnavExpand('Menu_on_" + studySubjectLabel + "_" + crf.getId() + "_" + rowCount
				+ "'); ";
		String href2 = "javascript:leftnavExpand('Menu_off_" + studySubjectLabel + "_" + crf.getId() + "_" + rowCount
				+ "'); ";
