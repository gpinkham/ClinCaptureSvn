...
public class UpdateStudySubjectServlet extends Controller {
	...
	private void confirm(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		FormDiscrepancyNotes discNotes = (FormDiscrepancyNotes) request.getSession()
				.getAttribute(AddNewSubjectServlet.FORM_DISCREPANCY_NOTES_NAME);
		...
		if (!errors.isEmpty()) {
			...
			if (StringUtil.isBlank(sub.getLabel())) {
				addPageMessage(respage.getString("must_enter_subject_ID_for_identifying")
						...
						+ respage.getString("study_subject_ID_should_not_contain_protected_information"), request);
			} else {
				StudySubjectDAO subdao = getStudySubjectDAO();
