...
public class ListDiscNotesForCRFTableFactory extends AbstractTableFactory {
	...
	protected void configureColumns(TableFacade tableFacade, Locale locale) {
		...
		configureColumn(row.getColumn(columnNames[2]), resword.getString("event_date"), new EventStartDateCellEditor(),
				null);
		...
		for (int i = 3; i < columnNames.length - 1; i++) {
			...
			configureColumn(row.getColumn(columnNames[i]), crfBean.getName(), new EventCrfCellEditor(),
					new SubjectEventCRFStatusDroplistFilterEditor(), false, false);
		}
	...
	public void configureTableFacadePostColumnConfiguration(TableFacade tableFacade) {
		ListDiscNotesForCRFTableToolbar toolbar = new ListDiscNotesForCRFTableToolbar(getStudyEventDefinitions(),
				selectedStudyEventDefinition);
	...
	public void setDataAndLimitVariables(TableFacade tableFacade) {
		...
		Collection<StudySubjectBean> items = getStudySubjectDAO().getWithFilterAndSort(getStudyBean(),
				listDiscNotesForCRF, eventsForSubjectSort, rowStart, rowEnd);
		...
		for (StudySubjectBean studySubjectBean : items) {
			...
			List<StudyEventBean> eventsForStudySubjectAndEventDefinitions = getStudyEventDAO()
					.findAllByDefinitionAndSubject(selectedStudyEventDefinition, studySubjectBean);
			...
			for (StudyEventBean studyEventBean : eventsForStudySubjectAndEventDefinitions) {
				...
				for (int i = 0; i < getCrfs(selectedStudyEventDefinition).size(); i++) {
					...
					if (eventCRFBean != null) {
						...
						List<DiscrepancyNoteBean> discs = getDiscrepancyNoteDAO()
								.findAllParentItemNotesByEventCRFWithConstraints(eventCRFBean.getId(), constraints);
	...
	protected ListDiscNotesForCRFFilter getListEventsForSubjectFilter(Limit limit) {
		ListDiscNotesForCRFFilter listDiscNotesForCRFFilter = new ListDiscNotesForCRFFilter(
	...
	private ArrayList<CRFBean> getCrfs(StudyEventDefinitionBean eventDefinition) {
		if (this.crfBeans == null) {
			...
			if (this.getStudyBean().getParentStudyId() > 0) {
				edcs = (List<EventDefinitionCRFBean>) getEventDefintionCRFDAO()
						.findAllActiveNonHiddenByEventDefinitionIdAndStudy(eventDefinition.getId(), this.getStudyBean());
			} else {
		...
		protected List<Option> getOptions() {
			...
			for (Object eventCRFStatus : DataEntryStage.toArrayList()) {
				if (((DataEntryStage) eventCRFStatus).getId() != 0) {
					options.add(new Option(String.valueOf(((DataEntryStage) eventCRFStatus).getId()),
							((DataEntryStage) eventCRFStatus).getName()));
				}
			}
			return options;
		}
	...
	private String viewStudySubjectLinkBuilder(StudySubjectBean studySubject) {
		...
		if (this.getResolutionStatus() >= 1 && this.getResolutionStatus() <= 5) {
			actionLink.a().href(
					"ViewNotes?viewForOne=y&module=" + module + "&id=" + studySubject.getId() + "&resolutionStatus="
							+ this.resolutionStatus + "&listNotes_f_studySubject.label=" + studySubject.getLabel());
			...
			actionLink.img().name("bt_View1").src("images/bt_View.gif").border("0").alt(resword.getString("view"))
					.title(resword.getString("view")).append("hspace=\"2\" style=\"float:left\" align=\"left\"").end()
					.aEnd();
			actionLink.append("&nbsp;&nbsp;&nbsp;");
		} else {
			actionLink.a().href(
	...
	private String downloadNotesLinkBuilder(StudySubjectBean studySubject) {
		...
		if (this.isStudyHasDiscNotes()) {
			if (this.getResolutionStatus() >= 1 && this.getResolutionStatus() <= 5) {
				...
				actionLink.a().href(
						"javascript:openDocWindow('ChooseDownloadFormat?discNoteType=" + discNoteType + "&module="
								+ module + "&subjectId=" + studySubject.getId() + "')");
				actionLink.img().name("bt_View1").src("images/bt_Download.gif").border("0")
						.alt(resword.getString("download_discrepancy_notes"))
						.title(resword.getString("download_discrepancy_notes")).append("hspace=\"2\"").end().aEnd();
				actionLink.append("&nbsp;&nbsp;&nbsp;");
