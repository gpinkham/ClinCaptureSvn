...
public class ListDiscNotesForCRFTableToolbar extends DefaultToolbar {
	...
	public ListDiscNotesForCRFTableToolbar(ArrayList<StudyEventDefinitionBean> studyEventDefinitions,
	...
	private class DownloadLinkItem extends AbstractItem {
		...
		public String enabled() {
			...
			html.a().href(
					"javascript:openDocWindow('ChooseDownloadFormat?resolutionStatus=" + resolutionStatus
							+ "&discNoteType=" + discNoteType + "&module=" + module + "')");
			html.quote();
			html.append(getAction());
			html.quote().close();
			html.img().name("bt_View1").src("images/bt_Download.gif").border("0")
	...
	private class StudyEventDefinitionDropDownItem extends AbstractItem {
		...
		public String enabled() {
			String js = "var selectedValue = document.getElementById('sedDropDown').options[document.getElementById('sedDropDown').selectedIndex].value;  "
					...
					+ " if (selectedValue != null && selectedValue == 0 ) { "
					+ "window.location='ListDiscNotesSubjectServlet?module=submit' } ";
			HtmlBuilder html = new HtmlBuilder();
