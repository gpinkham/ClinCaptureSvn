...
public class SiteStatisticsTableFactory extends AbstractTableFactory {
	...
	protected void configureColumns(TableFacade tableFacade, Locale locale) {
		...
		configureColumn(row.getColumn("expectedTotalEnrollment"), reswords.getString("expected_enrollment"), null,
				null, false, true);
		configureColumn(row.getColumn("percentage"), reswords.getString("percentage"), new PercentageCellEditor(),
				null, false, true);
	...
	public void setDataAndLimitVariables(TableFacade tableFacade) {
		...
		for (StudyBean studyBean : studies) {
			...
			Long percentage = expectedTotalEnrollment == 0 ? 0 : Math.round((countofStudySubjectsAtStudyOrSite
					.doubleValue() / expectedTotalEnrollment.doubleValue()) * 100);
