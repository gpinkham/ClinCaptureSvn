...
public class SpreadSheetTableClassic implements SpreadSheetTable {
	...
	public SpreadSheetTableClassic(FileInputStream parseStream, UserAccountBean ub, String versionName, Locale locale,
			int studyId, boolean isXlsx) throws IOException {
		// super();
		this.ub = ub;
		this.isXlsx = isXlsx;
		this.locale = locale;
			...
			if (sheetName.equalsIgnoreCase("Instructions")) {
				...
				if (sheetName.equalsIgnoreCase("Items")) {
					...
					for (int k = 1; k < numRows; k++) {
						...
						// Prevent parent names that equal the Item names
						if (itemName != null && itemName.equalsIgnoreCase(parentItem)) {
							parentItem = "";
						}

						cell = sheet.getRow(k).getCell(9);
						int columnNum = 0;
						...
						cell = sheet.getRow(k).getCell(10);
						if (cell != null) {
							page = getValue(cell);
						}

						cell = sheet.getRow(k).getCell(11);
						String questionNum = getValue(cell);

						cell = sheet.getRow(k).getCell(12);
						...
						cell = sheet.getRow(k).getCell(13);
						...
						if (!resNames.contains(responseLabel) && !StringUtil.isBlank(resOptions)) {
							// Only one option for "calculation"
						...
						if (responseTypeId == 8 || responseTypeId == 9) {
							...
							for (String v : variables) {
								if (!allItems.containsKey(v)) {
									errors.add(resPageMsg.getString("item") + v
											+ resPageMsg.getString("must_listed_before_item") + itemName
											+ resPageMsg.getString("item_at_row") + " " + k + ", "
											+ resPageMsg.getString("items_worksheet") + ". ");
									htmlErrors.put(j + "," + k + ",15", resPageMsg.getString("INVALID_FIELD"));
								}
							}
						...
						cell = sheet.getRow(k).getCell(20);
						...
						String codeRef = "";
						cell = sheet.getRow(k).getCell(27);
	...
	private String getMUInsertSql(String oid, String measurementUnitName, int ownerId, String dbName) {
		return "insert into measurement_unit (oc_oid, name) values ('" + oid + "', '"
