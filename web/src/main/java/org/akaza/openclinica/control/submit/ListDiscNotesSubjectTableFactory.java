...
public class ListDiscNotesSubjectTableFactory extends AbstractTableFactory {
	...
	public ListDiscNotesSubjectTableFactory(ResourceBundle resterm) {
		...
		discNoteIconPaths.put(1,
				"<img name='icon_Note' src='images/icon_Note.gif' border='0' alt='" + resterm.getString("Open")
						+ "' title='" + resterm.getString("Open") + "' align='left'/>");
		discNoteIconPaths.put(2, "<img name='icon_flagYellow' src='images/icon_flagYellow.gif' border='0' alt='"
				+ resterm.getString("Updated") + "' title='" + resterm.getString("Updated") + "' align='left'/>");
		discNoteIconPaths.put(3, "<img name='icon_flagGreen' src='images/icon_flagBlack.gif' border='0' alt='"
				+ resterm.getString("Resolved") + "' title='" + resterm.getString("Resolved") + "' align='left'/>");
		discNoteIconPaths.put(4, "<img name='icon_flagBlack' src='images/icon_flagGreen.gif' border='0' alt='"
				+ resterm.getString("Closed") + "' title='" + resterm.getString("Closed") + "' align='left'/>");
		discNoteIconPaths.put(5, "<img name='icon_flagWhite' src='images/icon_flagWhite.gif' border='0' alt='"
				+ resterm.getString("Not_Applicable") + "' title='" + resterm.getString("Not_Applicable")
				+ "' align='left'/>");
	...
	protected void configureColumns(TableFacade tableFacade, Locale locale) {
		...
		configureColumn(row.getColumn(columnNames[0]), currentStudy != null ? currentStudy.getStudyParameterConfig()
				.getStudySubjectIdLabel() : resword.getString("study_subject_ID"), null, null);
		configureColumn(row.getColumn(columnNames[1]), resword.getString("subject_status"), new StatusCellEditor(),
	...
	public void setDataAndLimitVariables(TableFacade tableFacade) {
		...
		Collection<StudySubjectBean> items = getStudySubjectDAO().getWithFilterAndSort(study, subjectFilter,
			...
			for (StudyEventDefinitionBean studyEventDefinition : getStudyEventDefinitions()) {
				...
				if (studyEvents.size() < 1) {
					...
					for (StudyEventBean studyEventBean : studyEvents) {
						discCounts = countAll(discCounts, studyEventBean, constraints,
								study.isSite(study.getParentStudyId()));
	...
	private class SubjectEventStatusDroplistFilterEditor extends DroplistFilterEditor {
		...
		protected List<Option> getOptions() {
			...
			for (Object subjectEventStatus : SubjectEventStatus.toArrayList()) {
				...
				options.add(new Option(String.valueOf(((SubjectEventStatus) subjectEventStatus).getId()),
						((SubjectEventStatus) subjectEventStatus).getName()));
			}
			return options;
		}
	...
	private class StudyEventDefinitionMapCellEditor implements CellEditor {
		...
		public Object getValue(Object item, String property, int rowcount) {
			...
			discCounts = (HashMap<ResolutionStatus, Integer>) ((HashMap<Object, Object>) item).get(property
					+ "_discCounts");
	...
	private String viewNotesLinkBuilder(StudySubjectBean studySubject) {
		...
		if (this.getResolutionStatus() >= 1 && this.getResolutionStatus() <= 5) {
			actionLink.a().href(
					"ViewNotes?viewForOne=y&id=" + studySubject.getId() + "&resolutionStatus=" + resolutionStatus
							+ "discNoteType=" + discNoteType + "&module=" + module + "&listNotes_f_studySubject.label="
			...
			actionLink.img().name("bt_View1").src("images/bt_View.gif").border("0").alt(resword.getString("view"))
					.title(resword.getString("view"))
					.append("hspace=\"4\" style=\"float:left\" width=\"24 \" height=\"15\" align=\"left\"").end()
					.aEnd();
			actionLink.append("&nbsp;&nbsp;&nbsp;");
		} else {
			actionLink.a().href(
					"ViewNotes?viewForOne=y&id=" + studySubject.getId() + "&module=" + module
							+ "&listNotes_f_studySubject.label=" + studySubject.getLabel());
			actionLink.append("onMouseDown=\"javascript:setImage('bt_View1','images/bt_View_d.gif');\"");
			actionLink.append("onMouseUp=\"javascript:setImage('bt_View1','images/bt_View.gif');\"").close();
			actionLink.img().name("bt_View1").src("images/bt_View.gif").border("0").alt(resword.getString("view"))
					.title(resword.getString("view"))
					.append("hspace=\"2\" style=\"float:left\" width=\"24 \" height=\"15\" align=\"left\"").end()
					.aEnd();
			actionLink.append("&nbsp;&nbsp;&nbsp;");
		}
	...
	private String downloadNotesLinkBuilder(StudySubjectBean studySubject) {
		...
		if (this.isStudyHasDiscNotes()) {
			if (this.getResolutionStatus() >= 1 && this.getResolutionStatus() <= 5) {
				actionLink
						.a()
						.href("javascript:openDocWindow('ChooseDownloadFormat?subjectId=" + studySubject.getId()
								+ "&discNoteType=" + discNoteType + "&resolutionStatus=" + resolutionStatus + "')")
						.close();
				actionLink.img().name("bt_Download").src("images/bt_Download.gif").border("0")
				...
				actionLink
						.a()
						.href("javascript:openDocWindow('ChooseDownloadFormat?subjectId=" + studySubject.getId()
								+ "&discNoteType=" + discNoteType + "&module=" + module + "')").close();
	...
	public HashMap<ResolutionStatus, Integer> countAll(HashMap<ResolutionStatus, Integer> discCounts,
		...
		temp = getDiscrepancyNoteDAO().countByEntityTypeAndStudyEventWithConstraints("itemData", studyEvent,
				constraints, isSite);
		this.getTotal(discCounts, temp);
		temp = getDiscrepancyNoteDAO().countByEntityTypeAndStudyEventWithConstraints("subject", studyEvent,
				constraints, isSite);
		this.getTotal(discCounts, temp);
		temp = getDiscrepancyNoteDAO().countByEntityTypeAndStudyEventWithConstraints("eventCrf", studyEvent,
				constraints, isSite);
		this.getTotal(discCounts, temp);
		temp = getDiscrepancyNoteDAO().countByEntityTypeAndStudyEventWithConstraints("StudySub", studyEvent,
				constraints, isSite);
		this.getTotal(discCounts, temp);
		temp = getDiscrepancyNoteDAO().countByEntityTypeAndStudyEventWithConstraints("studyEvent", studyEvent,
				constraints, isSite);
		this.getTotal(discCounts, temp);
	...
	public HashMap<ResolutionStatus, Integer> getTotal(HashMap<ResolutionStatus, Integer> discCounts,
			HashMap<ResolutionStatus, Integer> discCountsTemp) {
