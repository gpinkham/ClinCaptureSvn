...
/**
 * This class is used to generate the markup for a group-type printable table. An array or List of DisplaySectionBeans
 * is sequentially converted into XML to create a Web page representing all of the sections of a CRF. If needed, the
...
public class PrintHorizontalFormBuilder extends DefaultFormBuilder {
	...
	public String createMarkup() {
		...
		for (DisplaySectionBean displaySecBean : this.displaySectionBeans) {
			...
			if (studBean != null
					&& studBean.getStudyParameterConfig().getDiscrepancyManagement().equalsIgnoreCase("true")) {
			...
			for (DisplayItemGroupBean displayItemGroup : displaySecBean.getDisplayFormGroups()) {
				...
				if (hasDbFormValues) {
					currentDisplayItems = persistanceHandler.loadDataIntoDisplayBeans(currentDisplayItems,
							(!unGroupedTable));
					/*
					 * The highest number ordinal represents how many repeated rows there are. If the ordinal in
					 * ItemDataBeans > 1, then we know that the group has persistent repeated rows. Get a structure that
					 * maps each ordinal (i.e., >= 2) to its corresponding List of ItemDataBeans. Then iterate the
					 * existing DisplayBeans, with the number of new rows equaling the highest ordinal number minus 1
					 * (meaning, the first row represents the row of the group table that would exist if the user
					 * displayed the table, but didn't generate any new rows). For example, in a List of ItemDataBeans,
					 * if the highest ordinal property among these beans is 5, then the matrix table has 4 repeated rows
					 * from the database. Provide each new row with its values by using the ItemDataBeans.
					 */
					if (involvesDataEntry && !unGroupedTable
				...
				if (unGroupedTable) {
					orphanTable = formUtilDecorator.createXHTMLTableFromNonGroup(currentDisplayItems, tabindex,
							hasDiscrepancyMgt, hasDbFormValues, true);
				...
				// parameter
				List<Element> thTags = repeatFlag ? createTheadContentsFromDisplayItems(currentDisplayItems, true)
				...
				if (repeatFlag && !(involvesDataEntry && hasStoredRepeatedRows)) {
					table = repeatManager.addParentRepeatAttributes(table, repeatParentId, repeatNumber,
				...
				for (DisplayItemBean displayBean : currentDisplayItems) {
					...
					if (displayBean.getMetadata().getResponseLayout().equalsIgnoreCase("horizontal")
						...
						Element[] elements = cellFactory
								.createCellContentsForChecks(responseName, displayBean, displayBean.getMetadata()
										.getResponseSet().getOptions().size(), ++tabindex, false, true);
				...
				if (hasStoredRepeatedRows) {
					List storedRepeatedRows = builderUtil.generatePersistentMatrixRows(ordinalItemDataMap,
							currentDisplayItems, tabindex, repeatParentId, hasDiscrepancyMgt, true, maxColRow);
	...
	/**
	 * Sequentially create a String of XML representing all of the sections on a Case Report Form, for the purpose of
	 * web-page display.
	 * 
	...
	public String createMarkupNoDE() {
		...
		for (DisplaySectionBean displaySecBean : this.displaySectionBeans) {
			...
			for (DisplayItemGroupBean displayItemGroup : displaySecBean.getDisplayFormGroups()) {
				...
				// displayItemGroup.getItemGroupBean().getName().equalsIgnoreCase(BeanFactory.UNGROUPED);
				...
				for (DisplayItemBean displayBean : currentDisplayItems) {
					...
					if (displayBean.getMetadata().getResponseLayout().equalsIgnoreCase("horizontal")
						...
						for (Element el : elements) {
							el = builderUtil.setClassNames(el);
							if (repeatFlag) {
								el = repeatManager.addChildRepeatAttributes(el, repeatParentId, displayBean.getItem()
										.getId(), null);
							}
							row.addContent(el);
						}
	...
	public List<Element> createTheadContentsFromDisplayItems(List<DisplayItemBean> displayBeans,
