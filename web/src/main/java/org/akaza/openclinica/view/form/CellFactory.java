...
/**
 * The class responsible for adding different types of input elements or tags to an Element as content. The Element
 * represents an HTML "TD" or td tag. This class adds content to the cell such as input fields and select lists.
 * HorizontalFormBuilder uses this class to help build an HTML form, for instance.
 */
...
public class CellFactory {
	...
	/**
	 * Take an Element object and add various types of input tags to it as element content.
	 * 
	 * @param td
	 *            A JDOM Element object representing the td cell
	 * @param responseName
	 *            a String reprsenting the type of XHTML input as in text, textarea, or select
	 * @param displayBean
	 *            The DisplayItemBean associated with the input element.
	 * @param tabIndex
	 *            The tab index to use for the input element.
	 * @param hasDiscrepancyMgt
	 *            Specifies whether the discrepancy icon should be displayed in the form cell.
	 * @param hasSavedData
	 *            A boolean value specifying whether the input elements should be pre-filled with database values.
	 * @param forPrinting
	...
	public Element createCellContents(Element td, String responseName, DisplayItemBean displayBean, Integer tabIndex,
		...
		if (responseName.equalsIgnoreCase("text")) {
			td = inputGenerator.createTextInputTag(td, displayBean.getItem().getId(), tabIndex, displayBean
					.getMetadata().getDefaultValue(), (displayBean.getItem().getItemDataTypeId() == 9), displayBean
					.getData().getValue(), hasSavedData);
		} else if (responseName.equalsIgnoreCase("textarea")) {
			td = inputGenerator.createTextareaTag(td, displayBean.getItem().getId(), tabIndex, displayBean.getData()
					.getValue(), displayBean.getMetadata().getDefaultValue(), hasSavedData);

		} else if (responseName.equalsIgnoreCase("checkbox")) {
			td = inputGenerator.createCheckboxTag(td, displayBean.getItem().getId(), displayBean.getMetadata()
					.getResponseSet().getOptions(), tabIndex, true, displayBean.getData().getValue(), displayBean
					.getMetadata().getDefaultValue(), "horizontal".equalsIgnoreCase(displayBean.getMetadata()
					.getResponseLayout()), hasSavedData);

		} else if (responseName.equalsIgnoreCase("radio")) {
			td = inputGenerator.createRadioButtonTag(td, displayBean.getItem().getId(), displayBean.getMetadata()
					.getResponseSet().getOptions(), tabIndex, true, displayBean.getData().getValue(), displayBean
					.getMetadata().getDefaultValue(), "horizontal".equalsIgnoreCase(displayBean.getMetadata()
					.getResponseLayout()), hasSavedData);
			...
			td = inputGenerator.createSingleSelectTag(td, displayBean.getItem().getId(), displayBean.getMetadata()
					.getResponseSet().getOptions(), tabIndex, displayBean.getMetadata().getDefaultValue(), displayBean
					.getData().getValue(), hasSavedData);
		} else if (responseName.equalsIgnoreCase("multi-select")) {
			td = inputGenerator.createMultiSelectTag(td, displayBean.getItem().getId(), displayBean.getMetadata()
					.getResponseSet().getOptions(), tabIndex, displayBean.getData().getValue(), displayBean
					.getMetadata().getDefaultValue(), hasSavedData);
		} else if (responseName.equalsIgnoreCase("calculation") || responseName.equalsIgnoreCase("group-calculation")) {
			td = inputGenerator.createCaculationTag(td, displayBean.getItem().getId(), displayBean.getMetadata()
					.getResponseSet(), (displayBean.getItem().getItemDataTypeId() == 9), displayBean.getData()
					.getValue(), hasSavedData);
			...
			td = inputGenerator.createInstantTag(td, displayBean.getItem().getId(), tabIndex, displayBean.getData()
					.getValue(), hasSavedData);
		}
		...
		if (hasDiscrepancyMgt) {
			Element href;
			href = inputGenerator.createDiscrepancyNoteSymbol(displayBean.getNumDiscrepancyNotes(), tabIndex,
		...
		// cell
		if (responseName.equalsIgnoreCase("text") || responseName.equalsIgnoreCase("textarea")
	...
	public Element[] createCellContentsForChecks(String responseName, DisplayItemBean displayBean,
		...
		for (Iterator iter = displayBean.getMetadata().getResponseSet().getOptions().iterator(); iter.hasNext();) {
			...
			if (responseName.equalsIgnoreCase("checkbox")) {
				td = inputGenerator.createCheckboxTag(td, displayBean.getItem().getId(), list, tabIndex, false,
						displayBean.getData().getValue(), displayBean.getMetadata().getDefaultValue(), true,
						hasDBValues);
			} else {
				// Second to last true parameter is for horizontal radios
				td = inputGenerator.createRadioButtonTag(td, displayBean.getItem().getId(), list, tabIndex, false,
						displayBean.getData().getValue(), displayBean.getMetadata().getDefaultValue(), true,
						hasDBValues);
			}
			tdElements[indx] = td;
