...
public class HorizontalFormBuilder extends DefaultFormBuilder {
	...
	public String createMarkup() {
		...
		for (DisplayItemGroupBean displayItemGroup : this.displayItemGroups) {
			...
			boolean unGroupedTable = displayItemGroup.getItemGroupBean().getName()
					...
					|| !displayItemGroup.getGroupMetaBean().isRepeatingGroup();
			...
			if (hasDbFormValues) {
				 ...
				 * maps each ordinal (i.e., >= 2) to its corresponding List of ItemDataBeans. Then iterate the existing
				 * DisplayBeans, with the number of new rows equaling the highest ordinal number minus 1. For example,
				 * in a List of ItemDataBeans, if the highest ordinal property among these beans is 5, then the matrix
				 * table has 4 repeated rows from the database. Provide each new row with its values by using the
				 * ItemDataBeans.
				 */
			...
			if (unGroupedTable) {
				orphanTable = formUtil.createXHTMLTableFromNonGroup(currentDisplayItems, tabindex, hasDiscrepancyMgt,
						hasDbFormValues, false);
			...
			List<Element> thTags = repeatFlag ? createTheadContentsFromDisplayItems(currentDisplayItems, true,
					hasBorders) : createTheadContentsFromDisplayItems(currentDisplayItems, false, hasBorders);
			...
			for (DisplayItemBean displayBean : currentDisplayItems) {
				...
				if (displayBean.getMetadata().getResponseLayout().equalsIgnoreCase("horizontal")
					...
					Element[] elements = cellFactory.createCellContentsForChecks(responseName, displayBean, displayBean
							.getMetadata().getResponseSet().getOptions().size(), ++tabindex, false, false);
				...
				if (repeatFlag) {
					td = repeatManager
							.addChildRepeatAttributes(td, repeatParentId, displayBean.getItem().getId(), null);
				}
			...
			if (hasStoredRepeatedRows) {
				List<Element> storedRepeatedRows = builderUtil.generatePersistentMatrixRows(ordinalItemDataMap,
			...
			if (repeatFlag) {
				builderUtil.createAddRowControl(tbody, repeatParentId,
						(builderUtil.calcNumberofColumns(displayItemGroup) + 1), hasBorders);
	...
	public List<Element> createTheadContentsFromDisplayItems(List<DisplayItemBean> displayBeans,
			boolean generateExtraColumn, boolean hasDarkBorders) {
