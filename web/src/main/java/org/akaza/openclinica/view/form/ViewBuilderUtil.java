...
public class ViewBuilderUtil {
	...
	/**
	 * For the purpose of generating a certain HTML view, this method finds out whether any horizontally displayed
	 * checkboxes or radio buttons are involved.
	 * 
	 * @param displayBeans
	 *            A List of DisplayItemBeans for a CRF Section
	 * @return true if any of the DisplayItemBeans have a horizontal checkbox or radio button
	 */
	...
	public void addRemoveRowControl(Element row, String repeatParentId, boolean hasDarkBorders) {
		...
		repeatCell.addContent(RepeatManager.createrepeatButtonControl(ResourceBundleProvider.getResWord("remove"),
				repeatParentId));
	...
	public void createAddRowControl(Element tbody, String repeatParentId, int columnNumber, boolean hasDarkBorders) {
		...
		addButtonCell.addContent(RepeatManager.createrepeatButtonControl(ResourceBundleProvider.getResWord("add"),
				repeatParentId));
	...
	public void showTitles(Element divRoot, SectionBean sectionBean) {
		...
		if (divRoot == null || sectionBean.getInstructions().length() == 0 && sectionBean.getTitle().length() == 0
	...
	/**
	 * Generate a List of HTML rows (tr tag) for a matrix type table that already has form-field values from the
	 * database. In other words, the user previously created repeating rows and filled them with some data.
	 * 
	 * @param sortedDataMap
	 *            A SortedMap where the indiex is the Item Data's ordinal (like "2"), and the value is a List of
	 *            ItemDataBeans associated with that ordinal. These data make up the row's content.
	 * @param rowContentBeans
	 *            The DisplayItemBeans that provide the HTML fields for the rows.
	 * @param tabIndex
	 *            The incremented tab index for the form section.
	 * @param repeatParentId
	 *            The String id for the input name fields. See the RepeatManager class.
	 * @param hasDiscrepancyMgt
	 *            A boolean indicating whether the discrepancy note icon should be displayed.
	 * @param forPrinting
	 *            A boolean value indicating whether CRF printing is involved (if true, then discrepancy note icons are
	 *            not clickable).
	 * @param hasDarkBorders
	 * @return A List of row Elements, as in new Element("tr"); containing the previously generated repeating rows.
	 */
	public List<Element> generatePersistentMatrixRows(SortedMap<Integer, List<ItemDataBean>> sortedDataMap,
			List<DisplayItemBean> rowContentBeans, int tabIndex, String repeatParentId, boolean hasDiscrepancyMgt,
			boolean forPrinting, boolean hasDarkBorders) {
		...
		for (Integer ordinal : sortedDataMap.keySet()) {
			...
			for (DisplayItemBean disItemBean : rowContentBeans) {
				...
				if (disItemBean.getMetadata().getResponseLayout().equalsIgnoreCase("horizontal")
					...
					Element[] elements = cellFactory.createCellContentsForChecks(responseName, disItemBean, disItemBean
							.getMetadata().getResponseSet().getOptions().size(), ++tabIndex, true, forPrinting);
					for (Element el : elements) {
						...
						if (repeatFlag) {
							el = repeatManager.addChildRepeatAttributes(el, repeatParentId, disItemBean.getItem()
									.getId(), forcedParentId);
						}
						tr.addContent(el);
					}
				...
				td = cellFactory.createCellContents(td, responseName, disItemBean, ++tabIndex, hasDiscrepancyMgt, true,
						forPrinting);
				// In this case, the parent id looks like parentId_1, etc.
				td = repeatManager.addChildRepeatAttributes(td, repeatParentId, disItemBean.getItem().getId(),
						forcedParentId);
				tr.addContent(td);
			}
	...
	public List generatePersistentMatrixRowsNew(SortedMap<Integer, List<ItemDataBean>> sortedDataMap,
			List<DisplayItemBean> rowContentBeans, int tabIndex, String repeatParentId, boolean hasDiscrepancyMgt,
			boolean forPrinting, boolean hasDarkBorders, int maxColRows) {
