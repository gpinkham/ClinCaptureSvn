...
/**
 * BeanFactory contains methods for generating beans representing non-persistent data. the data derives largely from
 * spreadsheets defining Case Report Forms. The code generally uses the beans for previewing a new CRF version.
 * 
...
public class BeanFactory {
	...
	protected static final Logger logger = LoggerFactory
			.getLogger("org.akaza.openclinica.control.managestudy.BeanFactory");
	...
	/**
	 * This method creates a List of DisplayItemGroupBeans. The DisplayItemGroupBeans are listed in the order their
	 * items are defined in the CRF spreadsheet. These beans include the items that are and are not associated with a
	 * group. If an item is defined in a spreadsheet but does not have a group label, that item is included in a
	 * DisplayItemGroupBean, but not linked to any groups (i.e., that DisplayItemGroupBean has an empty FormGroupBean
	 * object, with an empty String for a group label). For example, if the spreadsheet defines two items first, but
	 * does not assign them a group, then those items will make up the first DisplayItemGroupBean. The spreadsheet then
	 * might define a <em>group</em> of items followed by more items that are not part of a group. The former items will
	 * represent the second DisplayItemGroupBean. The third DisplayItemGroupBean will include the last collection of
	 * "orphaned" items, accompanied by an empty FormGroupBean object.
	 * 
	 * @param itemsMap
	 *            A Map containing rows of Item information from the spreadsheet. The items are in the order they were
	 *            defined in the spreadsheet.
	 * @param sectionLabel
	 *            A String specifying the name of the section we are displaying.
	 * @param groupsMap
	 *            A Map containing rows of Group information from the spreadsheet.
	 * @param crfName
	 *            The name of the CRF, a String.
	 * @see org.akaza.openclinica.bean.submit.ItemGroupBean
	 * @return A List of DisplayItemGroupBeans
	 */
	public List<DisplayItemGroupBean> createGroupBeans(Map<Integer, Map<String, String>> itemsMap,
			Map<Integer, Map<String, String>> groupsMap, String sectionLabel, String crfName) {
		...
		if (groupsMap == null || groupsMap.isEmpty() || sectionLabel == null || sectionLabel.length() < 1
				|| sectionLabel.equalsIgnoreCase(UNGROUPED) || itemsMap == null || itemsMap.isEmpty()) {
			return groupBeans;
		}
	...
	private DisplayItemGroupBean getLatestDisFormBeanForOrphanedItems(List<DisplayItemGroupBean> beans) {
		...
		Collections.sort(list, new Comparator<DisplayItemGroupBean>() {
			...
			public int compare(DisplayItemGroupBean displayFormGroupBean, DisplayItemGroupBean displayFormGroupBean1) {
				return displayFormGroupBean.getGroupMetaBean().getOrdinal()
						.compareTo(displayFormGroupBean1.getGroupMetaBean().getOrdinal());
			}
		});
		...
		for (Entry<Integer, Map<String, String>> entry : groupMap.entrySet()) {
			...
			if (groupValues.get("group_label").equalsIgnoreCase(groupLabel)) {
				...
				try {
					...
					// An arbitrarily large number allows infinite
	...
	/*
	 * Generate the list of children for a DisplayItemBean. Currently, this method must return an ArrayList, because
	 * that is the data type for an ItemFormMetaDataBean's children field.
	 */
	public ArrayList<DisplayItemBean> createChildren(List<DisplayItemBean> childrenItems, DisplayItemBean parent) {
		...
		Collections.sort(children, new Comparator<DisplayItemBean>() {
			...
			public int compare(DisplayItemBean displayItemBean, DisplayItemBean displayItemBean1) {
				return new Integer(displayItemBean.getMetadata().getColumnNumber()).compareTo(displayItemBean1
						.getMetadata().getColumnNumber());
			}
		});
		return children;
	}
	...
	/*
	 * Create an ItemFormMetadataBean from the values referred to by a Map.Entry object. The Map.Entry contains an
	 * integer pointing to a Map of Item values from the Sreadsheet template, as in (item_name: Item1, etc...}
	 */
	...
	/**
	 * Create an ItemFormMetadataBean from the values referred to by a Map object. The Map contains the Item values from
	 * the Sreadsheet template, as in (item_name: Item1, etc...}
	 * 
	 * @param map
	...
	public DisplaySectionBean createDisplaySectionBean(Map<Integer, Map<String, String>> map, String sectionTitle,
			String sectionLabel, String sectionSubtitle, String instructions, String crfName, int sectionBorders) {
		/*
		 * each map row or entry looks like this: 36: {subheader=, validation=, page_number=2.0, response_type=text,
		 * left_item_text=Other Body Sysytem/Site, response_options_text=, required=, response_values=,
		 * question_number=35.0, phi=0.0, validation_error_message=, header=, units=, data_type=ST, response_label=text,
	...
	public List<SectionBean> createSectionBeanList(Map<Integer, Map<String, String>> sectionsMap,
			Map<Integer, Map<String, String>> itemsMap, String crfName, Map<Integer, Map<String, String>> groupsMap) {
	...
	public List<DisplayItemBean> createDisplayItemBeansFromMap(Map<Integer, Map<String, String>> itemsMap,
			String crfName) {
