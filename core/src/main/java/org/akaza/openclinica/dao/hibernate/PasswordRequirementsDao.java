...
public class PasswordRequirementsDao {
	public static final String PWD_CHARS_MIN = "pwd.chars.min", PWD_CHARS_MAX = "pwd.chars.max",
			PWD_CHARS_SPECIALS = "pwd.chars.specials", PWD_CHARS_DIGITS = "pwd.chars.digits",
			PWD_CHARS_CASE_UPPER = "pwd.chars.case.upper", PWD_CHARS_CASE_LOWER = "pwd.chars.case.lower",
			PWD_CHANGE_REQUIRED = "pwd.change.required", PWD_EXPIRATION_DAYS = "pwd.expiration.days",
	...
	private final List<String> boolConfigKeys = asList(PWD_CHARS_CASE_LOWER, PWD_CHARS_CASE_UPPER, PWD_CHARS_DIGITS,
			PWD_CHARS_SPECIALS), intConfigKeys = asList(PWD_CHARS_MIN, PWD_CHARS_MAX, PWD_EXPIRATION_DAYS,
		...
		for (ConfigurationBean bean : beans) {
			...
			if (boolConfigKeys.contains(key)) {
				...
				try {
					...
					logger.warn("Invalid configuration key: " + key + "." + " Should be an integer, but is: " + value,
							ex);
				}
			}
		}
