...
public class UsageStatsServiceDAO extends AbstractDomainDao<LogUsageStatsBean> {
	...
	@Transactional
	public void savePageLoadTimeToDB(String className, String time) {
		LogUsageStatsBean logUsageBean = new LogUsageStatsBean();
		Timestamp ocTime = new Timestamp(new java.util.Date().getTime());

		logUsageBean.setParam_key(className);
		logUsageBean.setParam_value(time);
		logUsageBean.setUpdate_timestamp(ocTime);

		saveOrUpdate(logUsageBean);
	}
