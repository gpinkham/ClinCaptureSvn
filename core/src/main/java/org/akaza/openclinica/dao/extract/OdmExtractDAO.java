...
public class OdmExtractDAO extends DatasetDAO {
	...
	public void getStudyEventAndFormMetaOC1_3(int parentStudyId, int studyId, MetaDataVersionBean metadata,
			String odmVersion, boolean isIncludedSite) {
		...
		logger.debug("getStudyEventAndFormMetaOC1_3SQl= "
				+ this.getStudyEventAndFormMetaOC1_3Sql(parentStudyId, studyId, isIncludedSite));
	...
	protected String getItemGroupAndItemMetaOC1_3Sql(String crfVersionIds) {
		return "select cv.crf_id, cv.crf_version_id,"
				...
				+ " default_value, show_item from item_form_metadata where crf_version_id in (" + crfVersionIds
				+ "))ifm, item, response_set rs,"
