...
public class FilterDAO extends AuditableEntityDAO {
	...
	/**
	 * To be created with the header string and an array list of FilterObjectBeans, which contain the number, type and
	 * value of criteria to apply to the existing filter.
	 * 
	 * fundamental change 06-03: adding the new query formation will change the SQL so that
	 * 
	...
	public String genSQLStatement(String oldSQLStatement, String connector, ArrayList filterObjs) {
		...
		while (it.hasNext()) {
			...
			tailEnd = tailEnd + "(item_id = " + fob.getItemId() + " and value " + fob.getOperand() + " '"
					+ fob.getValue() + "'))";
		}
		if (oldSQLStatement != null) {
			sb.append(" and ");
	...
	/**
	 * Will generate an explanation stating that this filter will look for value x at question y AND value like z at
	 * question a OR value not like a at question c...
	 * 
	...
	public ArrayList genExplanation(ArrayList oldExplanation, String connector, ArrayList filterObjs) {
		...
		while (it.hasNext()) {
			FilterObjectBean fob = (FilterObjectBean) it.next();
			String answerLine = "A value " + fob.getOperand() + " " + fob.getValue() + " " + "for question "
					+ fob.getItemName();
