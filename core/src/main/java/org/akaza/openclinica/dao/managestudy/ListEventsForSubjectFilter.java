			...
			if (property.equals("studySubject.status")) {
				...
				if (value.equals("3") || value.equals("6")) { // DataEntryStage.INITIAL_DATA_ENTRY_COMPLETE
					...
					criteria += " and  se.study_EVENT_ID  in(select study_event_id from  event_crf ec,crf_version cv where "
							+ "ec.crf_version_id = cv.crf_version_id and crf_id= "
							+ crfId
							+ "  and ( DATE_COMPLETED is not NULL and date_validate_completed is null ) )"
							+ "and se.study_event_definition_id ="
							+ studyEventDefinitionId
				 ...
				 */
					...
					criteria += " AND ( ( SELECT count(*) FROM event_crf event_crf, crf_version crf_version WHERE study_event_id in  "
							...
							+ "WHERE se.study_subject_id=SS.SUBJECT_ID and se.study_event_definition_id = "
							+ studyEventDefinitionId
							+ "and se.study_event_definition_id= sed.study_event_definition_id  ) and crf_version.crf_id ="
							+ crfId
							+ " and  "
