 ...
 * @author Jun Xu
 * 
 *         NI NoInformation No information whatsoever can be inferred from this exceptional value. This is the most
 ...
 * 
 *         NAV temporarily unavailable Information is not available at this time but it is expected that it will be
 *         available later.
 * 
 *         OTH other The actual value is not an element in the value domain of a variable. (e.g., concept not provided
 ...
 * 
 *         MSK masked There is information on this item available but it has not been provided by the sender due to
 *         security, privacy or other reasons. There may be an alternate mechanism for gaining access to this
 *         information. Note: using this null flavor does provide information that may be a breach of confidentiality.
 *         Its primary purpose is for those circumstances where it is necessary to inform the receiver that the
 *         information does exist.
 * 
 *         NP not present Value is not present in a message. This is only defined in messages, never in application
 *         data! All values not present in the message must be replaced by the applicable default, or no-information
...
public class NullValue extends Term {
	...
	public static NullValue get(int id) {
		...
		if (!(t instanceof NullValue) || !t.isActive()) {
			return INVALID;
		} else {
			return (NullValue) t;
		}
