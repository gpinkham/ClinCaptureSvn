 ...
 * a method from CoreResources.java.
 * 
 * @author thickerson
 * 
 */
...
public class ExtractPropertyBean implements Serializable {
	...
	// Associating epBean with datasetId, since core resources to get around with
	// coreResources.findExtractPropertyBeanById, to overcome the problems of static method
	...
	/*
	 * we need to set the file location and generate a unique name for the output. unique tags include: $date, $type,
	 * $datetime however, if we generate it here, we run the risk of generating a file name before other variables are
	 * set, i.e. type. also, we can't run the risk of generating two datestamps which may be different. tbh, 9/22/2010
	 */
