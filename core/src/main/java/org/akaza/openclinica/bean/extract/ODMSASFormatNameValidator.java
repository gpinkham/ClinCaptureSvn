 ...
 * character.
 * <li>if it formats a character variable, it should start with a dollar sign, i.e. "$"
 ...
 * a data file, the last 3 characters will be changed as the pattern sequentialNumber-englishAlphabet. The size of
 * sequential number is 2.
 * </ul>
 * 
 * 
...
public class ODMSASFormatNameValidator {
	...
	/**
	 * Given a name, this methods returns a valid SASFormatName and it guarantees the uniqueness of this name
	 * 
	 * @param name
	 *            String
	 * @return String
	...
	public String getValidSASFormatName(String name, boolean isCharacter) {
		...
		String temp = isCharacter && !name.trim().startsWith("$") ? "$" + name.trim().toUpperCase() : name.trim()
				.toUpperCase();
		...
		if (Character.isDigit(c[c.length - 1])) {
			if (s.length() < this.nameMaxLength) {
				...
				s = Character.isDigit(c[this.nameMaxLength - 2]) ? s.substring(0, this.nameMaxLength - 1) + "X" : s
						.substring(0, mysize) + this.getNextSequentialString() + 'X';
			}
		}
		String s2 = s;
		int index = 0;
