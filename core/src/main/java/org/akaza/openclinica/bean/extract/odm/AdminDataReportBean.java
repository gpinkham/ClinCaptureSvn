...
public class AdminDataReportBean extends OdmXmlReportBean {
	...
	public void addOneLocation(LocationBean loc, String currentIndent) {
		StringBuffer xml = this.getXmlOutput();
		String indent = this.getIndent();
		xml.append(currentIndent + "<Location OID=\"" + StringEscapeUtils.escapeXml(loc.getOid()) + "\" Name=\""
				+ StringEscapeUtils.escapeXml(loc.getName()) + "\">");
		xml.append(nls);
		MetaDataVersionRefBean m = loc.getMetaDataVersionRef();
		xml.append(currentIndent + indent + "<MetaDataVersionRef StudyOID=\""
				+ StringEscapeUtils.escapeXml(m.getStudyOID()) + "\" MetaDataVersionOID=\""
				+ StringEscapeUtils.escapeXml(m.getElementDefOID()) + "\" EffectiveDate=\""
				+ new SimpleDateFormat("yyyy-MM-dd").format(m.getEffectiveDate()) + "\"/>");
		xml.append(nls);
