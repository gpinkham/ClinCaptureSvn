...
public class PdfProcessingFunction extends ProcessingFunction {
	 ...
	 * have a *.fo suffix) and then return a success/fail message. (non-Javadoc)
	 * 
	...
	public ProcessingResultType run() {
		...
		try {
			...
			xslFile = new File(this.getTransformFileName());// transformfilename is abs path+file name(.fo)
															// transformFileName and odmxmlfile name are same?
		...
		resultSuccess.setUrl(CoreResources.getField("sysURL.base") + "AccessFile?fileId="); // to the pdf
