...
public class I18nFormatUtil {
	...
	public final static String dateFormatString(Locale locale) {
		Locale l = resolveLocale(locale);
		return StringUtil.parseDateFormat(ResourceBundleProvider.getFormatBundle(l).getString("date_format_string"));
	}
	...
	public final static SimpleDateFormat getDateTimeFormat(Locale locale) {
		Locale l = resolveLocale(locale);
		return new SimpleDateFormat(ResourceBundleProvider.getFormatBundle(l).getString("date_time_format_string"), l);
	}
	...
	public final static String yearMonthFormatString(Locale locale) {
		Locale l = resolveLocale(locale);
		return StringUtil
				.parseDateFormat(ResourceBundleProvider.getFormatBundle(l).getString("date_format_year_month"));
	}
