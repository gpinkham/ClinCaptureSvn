...
public class InstantOnChangeService {
	...
	public Map<Integer, InstantOnChangeFrontStrParcel> instantOnChangeFrontStrParcelInCrfVersion(Integer crfVersionId) {
		...
		Map<Integer, List<InstantOnChangePairContainer>> insMap = getItemFormMetadataDAO()
				.sectionInstantMapInSameSection(crfVersionId);
	 ...
	 */
	public void addToRepOrigins(Map<String, Map<Integer, InstantOnChangeFrontStrGroup>> repOrigins,
			InstantOnChangePairContainer instantPair) {
		String oigOid = instantPair.getOriginItemGroupOid();
		Integer oigItemId = instantPair.getOriginItemId();
		if (oigOid.length() > 0 && oigItemId > 0 && instantPair.getDestItemId() > 0
				&& instantPair.getOptionValue().length() > 0) {
	...
	public void addToNonRepOrigins(Map<Integer, InstantOnChangeFrontStrGroup> nonRepOrigins,
			InstantOnChangePairContainer instantPair) {
		Integer oigItemId = instantPair.getOriginItemId();
			...
			if (allItems.get(i).isInGroup()) {
				...
				for (DisplayItemGroupBean dig : digs) {
					if (repOrigins.containsKey(oid)) {
						HashMap<Integer, InstantOnChangeFrontStrGroup> oMap = (HashMap<Integer, InstantOnChangeFrontStrGroup>) repOrigins
								.get(oid);
	...
	public void itemsInstantUpdate(List<DisplayItemWithGroupBean> allItems,
			HashMap<Integer, InstantOnChangeFrontStrGroup> nonRepOrigins) {
		...
		for (int i = 0; i < allItems.size(); ++i) {
			if (!allItems.get(i).isInGroup()) {
				...
				if (nonRepOrigins.containsKey(id)) {
					...
					idSet.add(id);
					if (idSet.size() == nonRepOrigins.size())
						break;
