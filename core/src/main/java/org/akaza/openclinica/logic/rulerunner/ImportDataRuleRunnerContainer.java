...
public class ImportDataRuleRunnerContainer {
	...
	/**
	 * Populate importDataTrueRuleSets and variableAndValue. Precondition: import data file passed validation which
	...
	public void initRuleSetsAndTargets(DataSource ds, StudyBean studyBean, SubjectDataBean subjectDataBean,
		...
		this.importDataTrueRuleSets = this.importDataTrueRuleSets == null ? new ArrayList<RuleSetBean>()
				: this.importDataTrueRuleSets;
		...
		logger.debug("--- within container " + studyOid + " " + studySubjectOid + " ---");
		for (StudyEventDataBean studyEventDataBean : studyEventDataBeans) {
			...
			for (FormDataBean formDataBean : formDataBeans) {
				...
				StudyEventBean studyEvent = (StudyEventBean) new StudyEventDAO(ds)
						.findByStudySubjectIdAndDefinitionIdAndOrdinal(studySubject.getId(), sed.getId(), sedOrdinal);
				List<RuleSetBean> ruleSets = ruleSetService.getRuleSetsByCrfStudyAndStudyEventDefinition(studyBean,
						sed, crfVersion);
				...
				if (ruleSets != null && !ruleSets.isEmpty()) {
					...
					if (ruleSets != null && !ruleSets.isEmpty()) {
						...
						ruleSets = ruleSetService.filterRuleSetsByStudyEventOrdinal(ruleSets, studyEvent, crfVersion,
								sed);
							...
							for (ImportItemGroupDataBean itemGroupDataBean : itemGroupDataBeans) {
								...
								for (ImportItemDataBean importItemDataBean : itemDataBeans) {
									...
									if (isRepeatIGForSure(ds, crfVersion.getId(), igOid, igOrdinal, item.getId())) {
										...
										variableAndValue.put(importItemDataBean.getItemOID(),
												importItemDataBean.getValue());
