...
public class DataEntryRuleRunner extends RuleRunner {
	...
	public MessageContainer runRules(UserAccountBean ub, List<RuleSetBean> ruleSets, HashMap<String, String> variableAndValue) {
			...
			for (RuleSetBean ruleSet : ruleSets) {
				...
				for (ExpressionBean expressionBean : ruleSet.getExpressions()) {
					...
					for (RuleSetRuleBean ruleSetRule : ruleSet.getRuleSetRules()) {
						...
						ExpressionObjectWrapper eow = new ExpressionObjectWrapper(ds, currentStudy,
								rule.getExpression(), ruleSet, variableAndValue, ecb);
						try {
                            OpenClinicaExpressionParser oep = new OpenClinicaExpressionParser(currentStudy, request, eow);
                            String result = oep.parseAndEvaluateExpression(rule.getExpression().getValue());
							...
							if (itemData != null) {
                                ...
                                while (itr.hasNext()) {
                                    if (doesTheRuleActionRunLogExist(itemData, ruleSetRule, ruleSet, itr.next(), variableAndValue)) {
                                        itr.remove();
                                    }
                                }
							}
                            ...
                            if (code != null && (code.equalsIgnoreCase("OCRERR_CANT_GET_SUBJEC_DOB") || code.equalsIgnoreCase("OCRERR_CANT_GET_SUBJECT_ENROLLMENT"))) {
    ...
    private boolean doesTheRuleActionRunLogExist(ItemDataBean itemData, RuleSetRuleBean ruleSetRule, RuleSetBean ruleSet, RuleActionBean ruleActionBean, HashMap<String, String> variableAndValue) {
        ...
        if (ruleActionBean.getActionType() == ActionType.INSERT) {
            ...
            if (phase == RuleActionRunBean.Phase.DOUBLE_DATA_ENTRY && itemData.getStatus().getId() == 4 && request.getAttribute(firstDDE) == null) {
    ...
    private void showCustomErrors(RuleSetRuleBean ruleSetRule, RuleSetBean ruleSet, HashMap<String, String> variableAndValue, String customMessage) {
        for (RuleActionBean ruleActionBean : ruleSetRule.getActions()) {
            if (ruleActionBean.getRuleSetRule() == ruleSetRule && ruleActionBean.getRuleActionRun().canRun(phase) && ruleActionBean instanceof DiscrepancyNoteActionBean) {
                ItemDataBean itemData = getExpressionService().getItemDataBeanFromDb(ruleSet.getTarget().getValue());
                if (itemData == null || !doesTheRuleActionRunLogExist(itemData, ruleSetRule, ruleSet, ruleActionBean, variableAndValue)) {
                    ((DiscrepancyNoteActionBean)ruleActionBean).setCustomMessage(customMessage);
                    RuleActionContainer ruleActionContainer = new RuleActionContainer(ruleActionBean, ruleSet.getTarget(), itemData, ruleSet);
                    allActionContainerListBasedOnRuleExecutionResult.add(ruleActionContainer);
                }
            }
        }
