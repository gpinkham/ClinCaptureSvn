...
public class ScoreCalculator {
	...
	public String doCalculation(DisplayItemBean displayItem, HashMap<String, ItemBean> items,
			HashMap<String, String> itemdata, HashMap<Integer, TreeSet<Integer>> itemOrdinals, StringBuffer errs,
			int ordinal) {
		if (itemdata == null) {
		...
		if (type == 8) {
			...
			// For group-calculation type. Current restrictions:
	...
	/**
	 * Re-do calculations if funcs include changed item(s) and funcs are not included in the current section. If
	 * calculation can not sucessfully redo, old value will be erased and "<erased>" will be saved in database. <br>
	...
	public ArrayList<String> redoCalculations(HashMap<String, ItemBean> items, HashMap<String, String> itemdata,
		...
		try {
				...
				for (ItemFormMetadataBean ifmb : derivedItemList) {
					if (ifmb.getSectionId() != sectionId) {
						...
						for (int i = 0; i < groupsize; ++i) {
							ItemDataBean idb = iddao.findByItemIdAndEventCRFIdAndOrdinal(ifmb.getItemId(), ecb.getId(),
									i + 1);
							...
							if (p.isChanged(changedItems, parsedExp)) {
								...
								if (parser.getErrors().length() > 0) {
									...
									if (idb.isActive()) {
										...
										if (!idb.isActive()) {
											String key = i + 1 > 1 ? ifmb.getLeftItemText() + "_" + (i + 1) : ifmb
													.getLeftItemText();
			...
			List<ItemFormMetadataBean> itemList = ifmdao.findAllByCRFVersionIdAndResponseTypeId(ecb.getCRFVersionId(),
				...
				for (ItemFormMetadataBean ifmb : itemList) {
					if (ifmb.getSectionId() != sectionId) {
						...
						if (p.isChanged(changedItems, parsedExp)) {
							...
							if (err.length() > 0) {
								errors.add("Item " + ifmb.getLeftItemText() + " contains calculation errors: "
										+ err.toString());
							}
						}
					}
	...
	protected boolean writeToDB(ItemBean ib, ItemFormMetadataBean ifm, ItemDataBean idb, String exp, String value,
			StringBuffer err) {
		ItemDataDAO iddao = new ItemDataDAO(sm.getDataSource());
		if (idb == null) {
	...
	protected String getMathContextValue(String value, ItemFormMetadataBean ifm, ItemDataType idt,
