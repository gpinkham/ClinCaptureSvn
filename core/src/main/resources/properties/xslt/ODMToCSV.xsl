...
<xsl:stylesheet version="2.0"
	...
	<xsl:variable name="evt-map">
		<xsl:for-each select="/ODM:ODM">
			<xsl:if test="ODM:ClinicalData/ODM:SubjectData/ODM:StudyEventData/@OC:Status">
        		<event-status />
      		</xsl:if>
			<xsl:if test="ODM:ClinicalData/ODM:SubjectData/ODM:StudyEventData/
          @OC:StudyEventLocation">
				<location />
			</xsl:if>
			<xsl:if test="ODM:ClinicalData/ODM:SubjectData/ODM:StudyEventData/@OC:StartDate">
				<start-date />
			</xsl:if>
			<xsl:if test="ODM:ClinicalData/ODM:SubjectData/ODM:StudyEventData/@OC:EndDate">
				<end-date />
			</xsl:if>
			<xsl:if test="ODM:ClinicalData/ODM:SubjectData/ODM:StudyEventData/@OC:SubjectAgeAtEvent">
				<subject-age />
			</xsl:if>
		</xsl:for-each>
	</xsl:variable>
	...
	<xsl:template match="/">
		...
		<xsl:for-each select="ODM:ODM">
          ...
          substring(@CreationDateTime, 1, 4)))" />
			...
			<xsl:copy-of select="$TAB" />
			<xsl:value-of select="translate($BaseStudy/ODM:GlobalVariables/ODM:StudyName, ',', ';')" />
			<xsl:copy-of select="$LF" />

			<xsl:text>Protocol ID:  </xsl:text>
			<xsl:copy-of select="$TAB" />
			<xsl:value-of select="translate($BaseStudy/ODM:GlobalVariables/ODM:ProtocolName, ',', ';')" />
			<xsl:copy-of select="$LF" />
		...
		<xsl:for-each select="ODM:ODM/ODM:ClinicalData/ODM:SubjectData">
			...
			<!-- Subject ID -->
			<xsl:value-of select="translate(@OC:StudySubjectID, ',', ';')" />
			<xsl:text></xsl:text>
			<xsl:copy-of select="$TAB" />
			<!-- Protocol ID -->
			<xsl:value-of select="translate(key('study-protocol',
              ...
              [@FormOID = current()/@id]" />
                ...
                'true' or @ItemGroupRepeatKey = current()/@rep)]" />
                  ...
                  [@ItemOID = current()/@id]" />
							...
							<xsl:copy-of select="$TAB" />
							<xsl:text></xsl:text>
							<xsl:value-of select="normalize-space(translate($item/@Value, ',', ';'))" />
							<xsl:text></xsl:text>
						</xsl:for-each>
						<xsl:text></xsl:text>
					</xsl:for-each>
	...
	<xsl:template mode="body" match="primary-id">
		...
		<xsl:value-of select="translate($subject/@OC:UniqueIdentifier, ',', ';')" />
		<xsl:copy-of select="$TAB" />
	</xsl:template>
	...
	<xsl:template mode="body" match="secondary-id">
		...
		<xsl:value-of select="translate($subject/@OC:SecondaryID, ',', ';')" />
		<xsl:copy-of select="$TAB" />
	</xsl:template>
	...
	<xsl:template mode="body" match="group">
		...
		<xsl:value-of select="translate($subject/OC:SubjectGroupData
	...
	<xsl:template mode="body" match="location">
		...
		<xsl:text></xsl:text>
		<xsl:copy-of select="$TAB" />
		<xsl:value-of select="translate($event/@OC:StudyEventLocation, ',', ';')" />
	</xsl:template>
	...
	<xsl:template mode="body" match="form/field">
		...
		<xsl:text></xsl:text>
		<xsl:copy-of select="$TAB" />
		<xsl:value-of select="translate($form/@OC:*[local-name() = current()/@code], ',', ';')" />
	</xsl:template>
