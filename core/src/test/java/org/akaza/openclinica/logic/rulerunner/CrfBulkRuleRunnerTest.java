...
public class CrfBulkRuleRunnerTest extends DefaultAppContextTest {

	private StudyBean currentStudy;
	private CRFBean crf;
	private CRFVersionBean crfVersion;
	private UserAccountBean currentUser;
	private CrfBulkRuleRunner ruleRunner;
	private List<RuleSetBean> ruleSets;
	private HashMap<String, String> variableAndValue;
	private HashMap<RuleBulkExecuteContainer, HashMap<RuleBulkExecuteContainerTwo, Set<String>>> crfViewSpecificOrderedObjects;
	...
	public void setUp() {
		...
		crfVersion = (CRFVersionBean) crfVersionDao.findByPK(EIGHT);
		ruleRunner = new CrfBulkRuleRunner(dataSource, "", "", new JavaMailSenderImpl());
	...
	private void setUpRuleSetObjects(List<RuleSetBean> ruleSets) {
		for (RuleSetBean ruleSet : ruleSets) {
			...
			ruleSet.setCrf(crf);
			ruleSet.setCrfVersion(crfVersion);
	...
	@Test
	public void testThatRunCrfBulkRuleReturnsAffectedSubject() {
		...
		crfViewSpecificOrderedObjects = ruleRunner.runRulesBulk(ruleSets, ExecutionMode.DRY_RUN, currentStudy,
				variableAndValue, currentUser);
		assertEquals(1, crfViewSpecificOrderedObjects.size());
	}

	@Test
	public void testThatRunCrfBulkRuleWithRepeatingGroupsReturnsAffectedSubject() {
		ruleSets = new ArrayList<RuleSetBean>();
